...
(function(f, define) {
    ...
    (function($, undefined) {
        ...
        var ObservableObject = Observable.extend({
            ...
            forEach: function(f) {
                for (var i in this) {
                    if (this.shouldSerialize !== undefined)
            ...
            set: function(field, value) {
                var that = this,
                    ...
                    composite = field.indexOf === undefined ? false : field.indexOf('.') >= 0,
                ...
                if (current !== value) {
                    ...
                    if (!isSetPrevented) {
                        if (!composite) {
                            if (that.wrap !== undefined)
                                value = that.wrap(value, field, function() {
                                    return that;
                                });
                        }
                        if (that._set !== undefined)
        ...
        Model.define = function(base, options) {
            ...
            var model, proto = extend({
                ...
                name, field, type, value, idx, length, fields = {},
                originalName, id = proto.id,
                functionFields = [];
            if (id) {
                proto.idField = id;
            }
        ...
        Query.filterExpr = function(expression) {
            var expressions = [],
                ...
                idx, length, filter, expr, fieldFunctions = [],
                operatorFunctions = [],
                field, operator, filters = expression.filters;
        ...
        Query.prototype = {
            ...
            sort: function(field, dir, comparer) {
                var idx, length, descriptors = normalizeSort(field, dir),
                    comparers = [];
            ...
            filter: function(expressions) {
                var idx, current, length, compiled, predicate, data = this.data,
                    fields, operators, result = [],
                    filter;
            ...
            groupBy: function(descriptor) {
                ...
                var field = descriptor.field,
                    ...
                    item, groupValue = accessor.get(sorted[0], field),
                    group = {
                        field: field,
                        value: groupValue,
                        items: []
        ...
        Query.process = function(data, options) {
            ...
            var query = new Query(data),
                ...
                total, filterCallback = options.filterCallback,
        ...
        var RemoteTransport = Class.extend({
            ...
            setup: function(options, type) {
                options = options || {};
                var that = this,
                    parameters, operation = that.options[type],
        ...
        function serializeRecords(data, getters, modelInstance, originalFieldNames, fieldNames) {
            var record, getter, originalName, idx, setters = {},
                length;
            for (idx = 0, length = data.length; idx < length; idx++) {
                record = data[idx];
        ...
        var DataSource = Observable.extend({
            ...
            _findRange: function(start, end) {
                var that = this,
                    ranges = that._ranges,
                    range, data = [],
                    skipIdx, takeIdx, startIndex, endIndex, rangeData, rangeEnd, processed, options = that.options,
                    remote = options.serverSorting || options.serverPaging || options.serverFiltering || options.serverGrouping || options.serverAggregates,
                    flatData, count, length;
        ...
        function inferTable(table, fields) {
            var tbody = $(table)[0].tBodies[0],
                ...
                idx, length, fieldIndex, fieldCount = fields.length,
                data = [],
                cells, record, cell, empty;
        ...
        function inferList(list, fields) {
            var items = $(list).children(),
                idx, length, data = [],
                record, textField = fields[0].field,
                urlField = fields[1] && fields[1].field,
                spriteCssClassField = fields[2] && fields[2].field,
                imageUrlField = fields[3] && fields[3].field,
                item, id, textChild, className, children;
