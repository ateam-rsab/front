    ...
    (function($, undefined) {
        ...
        var ComboBox = Select.extend({
            init: function(element, options) {
                ...
                if ($(that.element).attr('required') !== undefined)
                    $(that.input).attr('required', $(that.element).attr('required'));
                disabled = $(that.element).parents('fieldset').is(':disabled');
                if (disabled) {
                    that.enable(false);
                }
                kendo.notify(that);
                this.$angular_scope.$watch(this.element.attr('k-ng-model'), function(e) {
                    if (e !== undefined) {
                        var key = that.element.attr('k-data-text-field').substring(1, that.element.attr('k-data-text-field').length - 1);
                        if (e.hasOwnProperty(key)) {
                            that.value(e[that.element.attr('k-data-text-field').substring(1, that.element.attr('k-data-text-field').length - 1)]);
            ...
            _selectValue: function(dataItem) {
                ...
                if ($(this.element).attr('close') !== undefined) {
                    this.$angular_scope[$(this.element).attr('close')]();
                }
            },
            refresh: function() {
