    ...
    (function($, undefined) {
        ...
        var MultiSelect = List.extend({
            init: function(element, options) {
                ...
                kendo.notify(that);
                that.$angular_scope.$watch($(that.element).attr('k-ng-model'), function(e) {
                    that.value(e);
                })
            },
            options: {
            ...
            _selectValue: function(added, removed) {
                ...
                if (that.$angular_scope[$(this.element).attr('k-ng-model')] === undefined)
                    that.$angular_scope[$(this.element).attr('k-ng-model')] = [];
                if (added.length !== 0) {
                    var arr = $(this.element).attr('k-ng-model').split('.');
                    var obj = that.$angular_scope;
                    for (var key in arr) {
                    ...
                    for (var key in added) {
                        if (added.hasOwnProperty(key)) {
                            var element = added[key];
                            var any = _.filter(obj, function(e) {
                                //var a = window.beforeSubmit(element.dataItem, true);
                                var a = element.dataItem;
                                if (JSON.stringify(e) === JSON.stringify(a))
                                    return e;
                            });
                            if (any.length === 0)
                                obj.push(element.dataItem);
                        }
                    }

                    if (that.$angular_scope.$$phase === null) {
                    ...
                    for (var key in removed) {
                        if (removed.hasOwnProperty(key)) {
                            var element = removed[key];
                            var index = obj.indexOf(element.dataItem);
                            obj.splice(index, 1);
                        }
                    }
