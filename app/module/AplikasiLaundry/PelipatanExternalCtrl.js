define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('PelipatanExternalCtrl', ['$rootScope', '$scope', '$state', 'ModelItem', 'DateHelper', 'ManageSarpras', 'FindSarpras', '$timeout', '$window',
		function ($rootScope, $scope, $state, ModelItem, dateHelper, ManageSarpras, FindSarpras, $timeout, $window) {
			...
			//State dari Daftar Pengeringan External
			$scope.noRecStrukPelayanan = $state.params.noRec;

			$scope.initPegawai = function () {
				debugger
				var tglawl = new moment(new Date($scope.TanggalAwal)).format('YYYY-MM-DD');
				var tglakhr = new moment(new Date($scope.TanggalAkhir)).format('YYYY-MM-DD');
				ManageSarpras.getOrderList("laundry/get-proses-pelipatan-external?startDate=" + tglawl + "&endDate=" + tglakhr).then(function (dat) {
					debugger
			...
			FindSarpras.getSarpras("laundry/get-produk-pelipatan").then(function (dat) {
			...
			$scope.DaftarPelipatan = function () {
				$state.go('DaftarPelipatanExternal')
			}
			...
			$scope.addDataPenerimaanLinen = function () {
				if ($scope.item.Linen.namaProduk == null || $scope.item.Linen == undefined) {
			...
			$scope.Save = function () {
				debugger
				$scope.NorecPelayanan = $state.params.noRec;
				...
				if (isValid.status) {
					...
					var data = {
						...
						"petugasId": $scope.item.petugasid,
						"jumlahCycle": $scope.item.totalLipat,
						"noRecStrukPelayanan": $scope.NorecPelayanan,
						"produkDetails": mapLinen
					}
					debugger
					ManageSarpras.savePengeringan(data, "laundry/save-proses-pelipatan-external").then(function (e) {
						$scope.initPegawai();
					});
				} else {
