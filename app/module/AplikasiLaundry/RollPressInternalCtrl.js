define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('RollPressInternalCtrl', ['$rootScope', '$scope', '$state', 'ModelItem', 'DateHelper', 'ManageSarpras', 'ManageLaundry', 'FindSarpras', 'FindLaundry', '$timeout', '$window',
		function ($rootScope, $scope, $state, ModelItem, DateHelper, ManageSarpras, ManageLaundry, FindSarpras, FindLaundry, $timeout, $window) {
			$scope.item = {};
			$scope.StatusKlik = true;
			$scope.isRouteLoading = false;
			$scope.now = new Date();
			debugger
			$scope.item.noRecStrukPelayanan = $state.params.noRecStrukPelayanan
			$scope.awal = new Date();
			$scope.akhir = new Date();
			...
			$scope.mainGridOptionsLipat = {
				...
				columns: [{
					field: "DaftarHasilPelipatan", title: "<h3 align=center>Daftar Hasil Pelipatan<h3>", headerAttributes: { style: "text-align : center" },
					columns: [
						{
							"field": "no",
							"title": "No.",
			...
			$scope.AddmainGridOptions = {
				...
				columns: [{
					field: "DaftarHasilPelipatan", title: "<h3 align=center>(+)Add Roll Press<h3>", headerAttributes: { style: "text-align : center" },
					columns: [{
						"field": "no",
			...
			$scope.selectRowPegawai = function (dataItem) {
				...
				if ($scope.NorecPegawai != undefined) {
					ManageLaundry.getOrderList("laundry/get-proses-rollpress-by-norec?noRec=" + $scope.NorecPegawai).then(function (dat) {
						$scope.SourceData = dat.data.data[0];
						if ($scope.SourceData.petugas[0] != undefined) {
							$scope.item.petugasdet = $scope.SourceData.petugas[0].namapegawai;
						}
			...
			$scope.Save = function () {
				...
				if (isValid.status) {
					...
					ManageLaundry.savePengeringan(data, "laundry/save-proses-rollpress").then(function (e) {
						$scope.initPegawai();
						window.location.href = '#/DaftarPackage'
					});
				} else {
