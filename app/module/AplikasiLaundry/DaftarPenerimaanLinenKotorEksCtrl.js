define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('DaftarPenerimaanLinenKotorEksCtrl', ['$rootScope', '$scope', 'ModelItem', 'DateHelper', 'ManageLaundry', '$state',
		function ($rootScope, $scope, ModelItem, dateHelper, ManageLaundry, $state) {
			$scope.item = {};
			$scope.now = new Date();
			...
			$scope.Proses = function () {
				...
				if ($scope.item.awal == undefined || $scope.item.akhir == undefined) {
					...
					ManageLaundry.getOrderList("laundry/get-penerimaan-linen-external-by-periode?startDate=" + dateHelper.formatDate($scope.item.awal, 'YYYY-MM-DD') + "&endDate=" + dateHelper.formatDate($scope.item.akhir, 'YYYY-MM-DD')).then(function (dat) {
			...
			$scope.Clicker = function (current) {
				debugger
				$scope.NorecTrukPelayanan = current.noRec
			}

			$scope.CuciLinen = function () {
				debugger
				if ($scope.NorecTrukPelayanan) {
					$state.go('PencucianLinenEksternal2', {
						noRec: $scope.NorecTrukPelayanan
					})
				} else {
					window.messageContainer.error('Data Harus dipilih terlebih dahulu!')
				}
...
/*		    $scope.Save = function() {
            	ManageLaundry.saveDaftarPenerimaanLinen($scope.dataPost,"laundry/send-penerimaan-to-pencucian").then(function (e) {
