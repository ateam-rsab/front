define(['initialize'], function(initialize) {
	'use strict';
	initialize.controller('PelaksanaanPencegahanJatuhPasienAnakCtrl', ['$rootScope', '$scope', 'ModelItem', 'DateHelper', 'FindPasien', '$state',
		function($rootScope, $scope, ModelItem, dateHelper, findPasien, $state) {
			$scope.cek = "PelaksaanPencegahanJatuhPasienAnakCtrl";
			...
			{
				strDataUrl  = "Pasien/PelaksanaanPencegahan/DataPelaksanaanPencegahanAnakStandarResikoTinggi";
			}
			else
			{
				strDataUrl  = "Pasien/PelaksanaanPencegahan/DataPelaksanaanPencegahanAnakStandarResikoRendah";
			}
			...
			$scope.item = {};
			ModelItem.get("Pasien/PelaksanaanPencegahanJatuhPasienAnak").then(function(data) {
                $scope.item = data;
                /*findPasien.getDataPelaksanaanPencegahanJatuhPasienAnak($state.params.noCM, $state.params.tanggal).then(function(e) {
                    $scope.item = e.data;
                ...
                $scope.item.penilaianPencegahanAnak = [];
			...
			$scope.chooseKriteria = function(data, child)
			{
				if($scope.isEditMode)
				{
					var currentData = _.find($scope.item.penilaianPencegahanAnak, function(arr){ return arr.parameter.id == data.id; });
					var indexArray = _.indexOf($scope.item.penilaianPencegahanAnak, currentData);

					$scope.item.penilaianPencegahanAnak[indexArray].parameter = data;
					$scope.item.penilaianPencegahanAnak[indexArray].kriteria = child;
				...
				else
				{
					...
					$scope.item.penilaianPencegahanAnak.push(dataPelaksanaanPencegahanJatuh);
			...
			$scope.backToPenilaian = function()
			{
				...
				$scope.item.penilaianPencegahanAnak.pop();
			...
			$scope.Save = function()
			{
				...
				for(var i=0; i < $scope.item.penilaianPencegahanAnak.length; i++)
				{
					delete $scope.item.penilaianPencegahanAnak[i].parameter.children;
					delete $scope.item.penilaianPencegahanAnak[i].parameter.statVisible;
					delete $scope.item.penilaianPencegahanAnak[i].parameter.score;
					delete $scope.item.penilaianPencegahanAnak[i].kriteria.score;
				}
