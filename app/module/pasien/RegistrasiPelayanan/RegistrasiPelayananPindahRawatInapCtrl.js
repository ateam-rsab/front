define(['initialize', 'Configuration'], function(initialize, configuration) {
    'use strict';
    initialize.controller('RegistrasiPelayananPindahRawatInapCtrl', ['ManagePasien', '$scope', 'ModelItem', '$state', '$rootScope', '$timeout', '$window','FindPasien', 'FindPegawai', 'DateHelper', 'CetakHelper',
        function(managePasien, $scope, modelItem, $state, $rootScope, $timeout, $window, findPasien, findPegawai, dateHelper, cetakHelper) { // form registrasi pelayanan untuk rawat inap duplicate from RegistrasiPelayananCtrl
            var responData;
            $scope.item = {};
            $scope.model = {};
            $scope.jenisPasiens = [{
            ...
            $scope.Simpan = function() {
                ...
                if ($scope.item.kelompokPasien.kelompokPasien.indexOf("Umum") < 0){
                    ...
                    if($scope.item.asalRujukan.asalRujukan.indexOf('Datang Sendiri') >= 0) listRawRequired.push("model.noRujukan|ng-model|No Rujukan");
                    var isValid = modelItem.setValidation($scope, listRawRequired);
                    if(isValid.status){
