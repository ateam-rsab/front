define(["initialize"], function (initialize) {
  "use strict";
  initialize.controller("DaftarRegistrasiPasienLabRadCtrl", [
    "$mdDialog",
    "$timeout",
    "$state",
    "$q",
    ...
    function (
      ...
      function loadCombo() {
        ...
        manageTataRekening
          .getDataTableTransaksi("operator/get-data-combo-operator", false)
          .then(function (data) {
        ...
        modelItemAkuntansi
          .getDataDummyPHP("operator/get-data-diagnosa", true, true, 10)
          .then(function (data) {
            // debugger;
      ...
      $scope.columnDaftarPasien = {
        columns: [
          ...
          },
          {
            ...
            width: "80px",
            template:
              "<span class='style-left'>{{formatTanggal('#: tglregistrasi #')}}</span>",
          },
          {
            field: "noregistrasi",
            title: "NoReg",
          ...
          },
          {
            ...
            width: "150px",
            template:
              "# if( namadokter==null) {# - # } else {# #= namadokter # #} #",
          },
          {
          ...
          },
          {
            ...
            width: "100px",
            template:
              "# if( namarekanan==null) {# - # } else {# #= namarekanan # #} #",
          },
          {
            field: "tglpulang",
            title: "Tgl Pulang",
            width: "80px",
            template:
              "# if( tglpulang==null) {# - # } else {# #= tglpulang # #} #",
          },
          {
            field: "nostruk",
            title: "No Verif",
      ...
      function loadData() {
        ...
        var tglAwal = moment($scope.item.periodeAwal).format(
          "YYYY-MM-DD HH:mm:ss"
        );
        var tglAkhir = moment($scope.item.periodeAkhir).format(
          "YYYY-MM-DD HH:mm:ss"
        );
        ...
        $q.all([
          ...
          for (var i = 0; i < result.length; i++) {
            ...
            result[i].umur =
              umur.year + " thn " + umur.month + " bln " + umur.day + " hari";
          }
          $scope.dataDaftarPasien = {
            ...
            //    },
            // ],
            aggregate: [{ field: "noregistrasi", aggregate: "count" }],
          };
      ...
      $scope.PasienPulang = function () {
        var tglpulang = moment($scope.item.tanggalPulang).format(
          "YYYY-MM-DD HH:mm:ss"
        );
      ...
      $scope.simpantglpulang = function () {
        ...
        manageTataRekening
          .saveupdatetglpulang(updateTanggal)
          .then(function (e) {
            loadData();
          });
        $scope.cbopasienpulang = false;
      ...
      $scope.cetakKartu = function () {
        ...
        if ($scope.dataPasienSelected.noregistrasi == undefined) var noReg = "";
        else var noReg = $scope.dataPasienSelected.noregistrasi;
        var stt = "false";
      ...
      $scope.pilihQty = function (data) {
        var listRawRequired = ["dats.qty|k-ng-model|kuantiti"];
      ...
      $scope.CetakSEP = function () {
        if (
          $scope.dataPasienSelected.noregistrasi != undefined &&
          $scope.dataPasienSelected.kelompokpasien !== "Umum/Pribadi"
        ) {
      ...
      $scope.RMK = function () {
        ...
        if ($scope.dataPasienSelected.noregistrasi != undefined) {
          manageTataRekening
            .getDataTableTransaksi(
              "operator/get-data-diagnosa-pasien?" + norReg
            )
            .then(function (data) {
      ...
      $scope.cetakRMK = function () {
        ...
        manageTataRekening
          .getDataTableTransaksi("operator/get-data-diagnosa-pasien?" + norReg)
          .then(function (data) {
            ...
            var detaildiagnosapasien = {
              ...
              tglpendaftaran: moment(
                $scope.dataPasienSelected.tglregistrasi
              ).format("YYYY-MM-DD hh:mm:ss"),
              tglinputdiagnosa: moment($scope.now).format(
                "YYYY-MM-DD hh:mm:ss"
              ),
              keterangan: keterangan,
            };
            manageTataRekening
              .postSaveDiagnosaRMK(detaildiagnosapasien)
              .then(function (e) {
      ...
      $scope.BatalPeriksa = function () {
        ...
        manageTataRekening
          .getDataTableTransaksi("operator/get-data-pasien-mau-batal?" + norReg)
          .then(function (data) {
            var datas = data.data;
            if (datas.length > 0 && datas[0].norec_pp == undefined) {
              $scope.item.ruanganBatal = {
                id: datas[0].objectruanganasalfk,
                namaruangan: datas[0].namaruangan,
              };
              ...
              window.messageContainer.error(
                "Pasien sudah Mendapatkan Pelayanan"
              );
      ...
      $scope.lanjutBatal = function () {
        ...
        manageTataRekening
          ...
          .then(function (data) {
            var BatalPeriksa = {
              norec: $scope.dataPasienSelected.norec,
              tanggalpembatalan: moment($scope.item.tglbatal).format(
                "YYYY-MM-DD hh:mm:ss"
              ),
            ...
            manageTataRekening
              .postSaveBatalRegistrasi(BatalPeriksa)
              .then(function (e) {
                loadData();
          ...
          manageTataRekening
            ...
            .then(function (data) {
              ...
              if (dataAntrian != undefined) {
                ...
                var cacheSet =
                  $scope.dataPasienSelected.objectasuransipasienfk +
                  "~" +
      ...
      $scope.transaksiPelayanan = function () {
        if (
          $scope.dataPasienSelected.kelompokpasien === "Umum/Pribadi" &&
          $scope.dataPasienSelected.nostruk &&
        ...
        var arrStr = {
          ...
          16: $scope.ruanganLogin.nosbm,
          17: $scope.ruanganLogin.nostruk,
        };
