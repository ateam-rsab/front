define(['initialize'], function(initialize) {
	'use strict';
	initialize.controller('DaftarRegistrasiPasienBayiCtrl', ['$mdDialog', '$timeout', '$state', '$q', '$rootScope', '$scope', 'CacheHelper','DateHelper','ManageTataRekening', 'ModelItem','CetakHelper','ManagePasien','ModelItemAkuntansi',
		function($mdDialog, $timeout, $state, $q, $rootScope, $scope, cacheHelper,dateHelper,manageTataRekening, ModelItem, cetakHelper, managePasien, modelItemAkuntansi) {
			...
			function loadCombo(){
				var chacePeriode = cacheHelper.get('DaftarRegistrasiPasienBayiCtrl');
			...
			$scope.columnDaftarPasienPulang = [
				...
				},
				{
					...
					"title": "Umur",
					"width":"150px",
					"template": "<span class='style-left'>#: umurzz #</span>"
				},
				{
					"field": "namaruangan",
					"title": "Nama Ruangan",
			...
			function loadData(){
				...
				$q.all([
					manageTataRekening.getDataTableTransaksi("operator/get-daftar-registrasi-bayi?"+
						"tglAwal="+tglAwal+
						"&tglAkhir="+tglAkhir+
                        ...
                        data[0].data[i].umurzz =umurzz.year + ' thn ' + umurzz.month + ' bln ' + umurzz.day + ' hari'
                    }
                    $scope.dataDaftarPasienPulang = new kendo.data.DataSource({
						...
						var chacePeriode = tglAwal + "~" + tglAkhir 
									 ...
									 // + "~" +$scope.item.dokter;
						cacheHelper.set('DaftarRegistrasiPasienBayiCtrl', chacePeriode);
					});
        ...
        $scope.CetakSEP = function() {
                if($scope.dataPasienSelected.noregistrasi != undefined && $scope.dataPasienSelected.kelompokpasien !== "Umum/Pribadi"){
                    ...
                    //cetakan langsung service VB6 by grh    
                    var client = new HttpClient();
                    client.get('http://127.0.0.1:1237/printvb/Pendaftaran?cetak-sep=1&norec=' + $scope.dataPasienSelected.noregistrasi + '&view=false', function(response) {
                        // do something with response
                    });
                }
        }
    ...
    //              	return;
            ...
            $scope.InputDetailBayi = function(){
            	debugger;
				...
				if ($scope.dataPasienSelected==undefined)
			 	{
			 		window.messageContainer.error("Pilih Data Pasien dulu!!!");
			 	}else{
			 		$state.go('SkriningGizi&TandaVitalRev',{
	                        norecPD:$scope.dataPasienSelected.norec
		          	});
		 		  	var CacheRuangan = $scope.dataPasienSelected.objectruanganlastfk                                     
                  	cacheHelper.set('CacheRuangan', CacheRuangan);
			 	}
            }
