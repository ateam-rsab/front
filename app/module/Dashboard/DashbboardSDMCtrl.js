define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('DashbboardSDMCtrl', ['$q', '$parse', 'LoginService', 'socket', '$rootScope', '$scope', 'ModelItem', '$state', 'DateHelper', 'ManageSdm', 'ManageSdmNew',
		'ReportHelper', 'CetakHelper', 'FindSdm', 'CetakHelper', '$timeout',
		function ($q, $parse, loginService, socket, $rootScope, $scope, ModelItem, $state, DateHelper, ManageSdm, ManageSdmNew, reportHelper, CetakHelper, FindSdm, cetakHelper,
			$timeout) {
			...
			var load = function () {
				ManageSdmNew.getListData("sdm/get-all-tindakan-by-period/" + $scope.item.pegawai.id).then(function (dat) {
					$scope.listTindakan = dat.data.data;
					// $scope.item.tindakan = dat.data.data[0];
					$scope.isRouteLoading = false;
				});

				let now = new Date();
				now = moment().subtract(2, 'months');
				$scope.validateMonth = now;
			}
			...
			$scope.cari = function () {
				...
				if ($scope.item.tindakan == undefined) {
					...
					idTindakan = $scope.item.tindakan.id;
				}

				$scope.isRouteLoading = true;

				search.goleti = function () {
					ManageSdmNew.getListData("sdm/get-persen-uraian-kerja/" + $scope.item.pegawai.id + "/" + awal + "/" + akhir).then(function (dat) {
				...
				search.find = function () {
					...
					ManageSdmNew.getListData("sdm/get-logbook-summary-by-user-id?idPegawai=" + $scope.item.pegawai.id + "&startDate=" + awal + "&endDate=" + akhir + "&idTindakan=" + idTindakan).then(function (dat) {
			...
			$scope.$watch('item.pegawai', function (newVal, oldVal) {
				...
				timeoutPromise = $timeout(function () {
					if (newVal && newVal !== oldVal) {
						ManageSdmNew.getListData("sdm/get-all-tindakan-by-period/" + newVal.id).then(function (dat) {
							$scope.listTindakan = dat.data.data;
						});
					}
				}, 1000);
			...
			$scope.getFeeService = function () {
				...
				if (isValid.status) {
					...
					FindSdm.getFeeForServiceDokter(DateHelper.getDateTimeFormatted3($scope.item.periodeFee), $scope.item.pegawai.id).then(function (dat) {
						...
						$scope.dataSourceFeeForService = new kendo.data.DataSource({
							data: dataGrid,
								aggregate: [
									{ field: "totalTindakan", aggregate: "sum" },
									{ field: "pointQty", aggregate: "sum" }
									...
									columns: [
										{
											...
											footerTemplate: "#= getAll(totalTindakan) #",
											footerAttributes: {
												"class": "table-footer-cell",
												style: "text-align: right;"
											}
			...
			// 			title: "Tarif (Rp.)", 
			// 			template: '# if( hargaKelas1 != null ) {# #= hargaKelas1# #} else {# #= harga# #} #', 
			// 			format: "{0:n0}", width: 100, headerAttributes: { style: "text-align: center" }, attributes: {
			// 				"class": "table-cell", style: "text-align: right;  "//font-size: 14px;"
			// 			}
			// 		},
			// 		// { field: "Pencapaian", headerAttributes: { style: "text-align: center"}, columns: $scope.generateKolomFeeService() },
