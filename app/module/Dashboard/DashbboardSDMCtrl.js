		...
		function ($q, $parse, loginService, socket, $rootScope, $scope, ModelItem, $state, DateHelper, ManageSdm, ManageSdmNew
			...
			$scope.cetakLogBook = function () {
				if ($scope.jenisCetakan == 'CetakLogbook') {
					$scope.cetakLogBookKinerja();
				} else if ($scope.jenisCetakan == 'CetakDaftarLogbook') {
					$scope.cetakDaftarLogBookKinerja();
				} else if ($scope.jenisCetakan == 'CetakDetailLogbook') {
					$scope.cetakDaftarLogBookKinerjaRekapWithPasien();
				} else if ($scope.jenisCetakan == 'CetakDaftarFFS') {
					$scope.cetakDaftarLogBookKinerjaFFS();
			...
			$scope.openWindowCetakLks = function (param) {
				...
				// if (DateHelper.getFormatMonthPicker($scope.item.periode) == '2022-10') {
			...
			$scope.openWindowCetakFfs = function (param) {
				if (!$scope.item.periodeFee) {
					return toastr.warning('Periode belum dipilih', 'Info');
				}

				// if (DateHelper.getFormatMonthPicker($scope.item.periodeFee) == '2022-10') {
				// 	return toastr.warning('OOps! Periode November 2022 Belum tersedia', 'Info');
				// } else {
				ManageSdmNew.getListData("pegawai/get-all-jabatan-by-pegawai?idPegawai=" + $scope.item.pegawai.id).then(function (res) {
					$scope.listJabatanCetak = res.data.data;
			...
			$scope.openWindowCetakLksFfs = function (param) {
				if (!$scope.item.periodeRekaps) {
					return toastr.warning('Periode belum dipilih', 'Info');
				}

				// if (DateHelper.getFormatMonthPicker($scope.item.periodeRekaps) == '2022-10') {
				// 	return toastr.warning('OOps! Periode November 2022 Belum tersedia', 'Info');
				// } else {
				ManageSdmNew.getListData("pegawai/get-all-jabatan-by-pegawai?idPegawai=" + $scope.item.pegawai.id).then(function (res) {
					$scope.listJabatanCetak = res.data.data;
					...
					ManageSdmNew.getListData("sdm/get-all-tindakan-dokter-rescored/" + DateHelper.getFormatMonthPicker($scope.item.periode) + "/" + $scope.item.pegawai.id + "/1").then(function (dat) {
						...
						$scope.mainGridOption = {
							...
							columns: [
								...
								{ field: "kelompokPasien", title: "Kelompok Pasien", width: 150 },
						...
						if (colName !== "") {
							...
							if (colName.length <= 2) {
								...
								if (gridId === "gridOrder") {
									...
									$scope.showDetail(selectedData.idProduk, selectedData.kelompokPasienId, selectedData.idKelas, $scope.item.pegawai.id, akhir, ffs, selectedData.diskon, selectedData.statusDiskon);
			...
			$scope.showDetail = function (idProduk, idKelompokPasien, idKelas, idPegawai, tgl, ffs, diskon, statusDiskon) {
				$scope.isRouteLoading = true;
				// FindSdm.getDetilLogbookKinerja(idProduk, idKelas, idPegawai, tgl, ffs).then(function(data){
				ManageSdmNew.getListData("sdm/get-detail-pasien/" + idProduk + "/" + idKelompokPasien + "/" + idKelas + "/" + idPegawai + "/" + tgl + "/" + ffs
			...
			$scope.cetakDaftarLogBookKinerja = function () {
				...
				var isValid = ModelItem.setValidation($scope, listRawRequired);
				if (isValid.status) {
					var fixUrlLaporan = cetakHelper.openURLReporting("reporting/lapLogbookPegawaiVer2?idPegawai=" + $scope.item.pegawai.id + "&idJabatan=" + $scope.item.jabatanCetak.id + "&idAtasan=" + $scope.item.atasanCetak.id + "&idJabatanAtasan=" + $scope.item.jabatanAtasanCetak.id + "&periode=" + DateHelper.getFormatMonthPicker($scope.item.periode) + "&isFFS=false");
					window.open(fixUrlLaporan, '', 'width=800,height=600')
				} else {
					ModelItem.showMessages(isValid.messages);
				}
			...
			$scope.cetakDaftarLogBookKinerjaFFS = function () {
				...
				var isValid = ModelItem.setValidation($scope, listRawRequired);
				if (isValid.status) {
					var fixUrlLaporan = cetakHelper.openURLReporting("reporting/lapLogbookPegawaiVer2?idPegawai=" + $scope.item.pegawai.id + "&idJabatan=" + $scope.item.jabatanCetak.id + "&idAtasan=" + $scope.item.atasanCetak.id + "&idJabatanAtasan=" + $scope.item.jabatanAtasanCetak.id + "&periode=" + DateHelper.getFormatMonthPicker($scope.item.periodeFee) + "&isFFS=true");
					window.open(fixUrlLaporan, '', 'width=800,height=600')
				} else {
					ModelItem.showMessages(isValid.messages);
				}
			...
			$scope.cetakDaftarLogBookKinerjaRekap = function () {
				...
				var isValid = ModelItem.setValidation($scope, listRawRequired);
				if (isValid.status) {
					var fixUrlLaporan = cetakHelper.openURLReporting("reporting/lapLogbookPegawaiRekap?idPegawai=" + $scope.item.pegawai.id + "&idJabatan=" + $scope.item.jabatanCetak.id + "&idAtasan=" + $scope.item.atasanCetak.id + "&idJabatanAtasan=" + $scope.item.jabatanAtasanCetak.id + "&periode=" + DateHelper.getFormatMonthPicker($scope.item.periodeRekaps));
					window.open(fixUrlLaporan, '', 'width=800,height=600')
				} else {
					ModelItem.showMessages(isValid.messages);
				}
