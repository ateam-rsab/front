define(['initialize'], function (initialize, pasienServices) {
    'use strict';
    initialize.controller('RiwayatRegistrasi3Ctrl', ['$q', 'ManagePasien', '$rootScope', '$scope', 'ModelItem', 'ModelItemAkuntansi', '$state', 'FindPasien', 'CacheHelper', 'DateHelper', 'ManageSarprasPhp',
        function ($q, managePasien, $rootScope, $scope, ModelItem,modelItemAkuntansi, $state, findPasien, cacheHelper, dateHelper, manageSarprasPhp) {
         ...
         manageSarprasPhp.getDataTableTransaksi("tatarekening/get-data-combo-daftarregpasien", false).then(function(data) {
            ...
            function loadData(){
                ...
                if($state.params.nocm != undefined){
                    var noCm ="noCm=" +$state.params.nocm
                }else {
                    var noCm ="noCm=" +$scope.item.noCm
                }

                var noReg =""
                if ($state.params.noregistrasi != undefined){
                    var noReg ="noReg=" +$state.params.noregistrasi
                }   
