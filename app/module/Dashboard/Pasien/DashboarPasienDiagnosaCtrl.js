define(['initialize'], function(initialize, pasienServices) {
    'use strict';
    initialize.controller('DashboarPasienDiagnosaCtrl', ['DateHelper', 'FindPasien', 'ManagePasien', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper', 'FindPegawai',
        ...
        function(dateHelper, findPasien, managePasien, $rootScope, $scope, ModelItem, $state, cacheHelper, findPegawai) {
            ...
            if ($state.current.name === 'dashboardpasien.PengkajianMedis.DashboardDiagnosis.DiagnosisICD10') {
                $rootScope.showMenu = false;
                findPegawai.getDiagnosaTop('P0001').then(function(data) {
                    $scope.topDiagnosis = data.data;
                });
            } else if ($state.current.name === 'dashboardpasien.PengkajianMedis.DashboardDiagnosis.DiagnosisICD10') {
                $rootScope.showMenuPengkajianMedis = true;
                // findPegawai.getDiagnosaTop('P0001').then(function(data) {
                //     $scope.topDiagnosis = data.data;
                // });
            ...
            if ($state.current.name === 'dashboardpasien.PengkajianMedis.DashboardDiagnosis.DiagnosisICD10' || $state.current.name === 'dashboardpasien.PengkajianMedis.Diagnosis') {
