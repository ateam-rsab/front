define(['initialize'], function(initialize, pasienServices) {
    'use strict';
    initialize.controller('DashboarPasienLayananMedikRawatInapCtrl', ['$rootScope', '$scope', 'ModelItem', '$state', 'FindPasien', 'CacheHelper', 'DateHelper', 'ManagePasien',
        function($rootScope, $scope, ModelItem, $state, findPasien, cacheHelper, dateHelper, ManagePasien) {
            ...
            findPasien.getPersetujuan($scope.noCM).then(function(data) {

                if (data != undefined) {
                    $scope.isNoRecFound = true;
                    var item = [];
                    ...
                    $scope.listDaftar = item;
                }
            });


            findPasien.getPelayananRawatInap($scope.noCM).then(function(data) {
                ...
                findPasien.getDetailSoap($scope.noRec).then(function(data){
                    ...
                    for (var key in data.data.data.soapSet) {
                        if (data.data.data.soapSet.hasOwnProperty(key)) {
                            ...
                            $scope.tglInput = new moment(element.tglInput).format('YYYY-MM-DD HH:mm:ss')
