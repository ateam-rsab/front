define(['initialize'], function (initialize) {
    ...
    initialize.controller('OrderBarangSPPBCtrl', ['$q', '$rootScope', '$scope', 'ManageLogistikPhp', '$state', 'CacheHelper', 'ModelItemAkuntansi',
        function ($q, $rootScope, $scope, manageLogistikPhp, $state, cacheHelper, modelItemAkuntansi) {
                    ...
                    if (noOrder != '') {
                        ...
                        if (noOrder == 'EditOrder') {
                            ...
                            manageLogistikPhp.getDataTableTransaksi("sppb/get-detail-SPPB?norecOrder=" + norecResep, true).then(function (data) {
                                ...
                                $scope.item.alamatSupl = data.data.detail.alamatrekanan
                                $scope.item.telpSupl = 'Telp. ' + data.data.detail.tlprekanan + ' Fax. ' + data.data.detail.faxrekanan
                                $scope.item.mataAnggaran = {
                                    norec: data.data.detail.mataanggranid,
                                    mataanggaran: data.data.detail.mataanggaran
                                };
                                // if(data.data.detail.norecrealisasi)norecrrsppb
            ...
            //         manageLogistikPhp.getDataTableTransaksi("sppb/get-data-sppb?NoSPPB=" + $scope.item.noOrder, true).then(function (data) {
            //             var datas = data.data;
            //             for (var i = datas.length - 1; i >= 0; i--) {
            //                 if (datas[i].noorder == $scope.item.noOrder) {
            //                     alert("No SPPB Tidak Boleh Sama!")
