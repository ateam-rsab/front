define(['initialize'], function(initialize) {
    'use strict';
    initialize.controller('OrderBarangSPPBCtrl', ['$q', '$rootScope', '$scope', 'ManageLogistikPhp','$state','CacheHelper','ModelItemAkuntansi',
                    ...
                    if (noOrder != '') {
                        ...
                        if (noOrder == 'EditOrder') {
                            ...
                            manageLogistikPhp.getDataTableTransaksi("sppb/get-detail-SPPB?norecOrder="+norecResep, true).then(function(data_ih){
                                ...
                                $scope.item.alamatSupl=data_ih.data.detail.alamatrekanan
                                $scope.item.telpSupl = 'Telp. ' + data_ih.data.detail.tlprekanan + ' Fax. ' + data_ih.data.detail.faxrekanan
                                $scope.item.mataAnggaran = {norec:data_ih.data.detail.mataanggranid, mataanggaran:data_ih.data.detail.mataanggaran};
                                // if(data_ih.data.detail.norecrealisasi)norecrrsppb
                                ...
                                for (var i = data2.length - 1; i >= 0; i--) {
                                    totTpnn=(parseFloat(data2[i].hargasatuan)*(data2[i].jumlah))
        ...
        $scope.$watch('item.noOrder', function(newValue, oldValue) {
            if (newValue != oldValue  ) {
                ...
                manageLogistikPhp.getDataTableTransaksi("sppb/get-data-sppb?NoSPPB="+$scope.item.noOrder, true).then(function(data_ih){
