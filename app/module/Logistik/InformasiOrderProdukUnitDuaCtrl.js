define(['initialize'], function(initialize) {
    ...
    initialize.controller('InformasiOrderProdukUnitDuaCtrl', ['$state', 'FindSarpras', '$rootScope', '$scope', 'ModelItem','ManageSarpras',
        function($state, findSarpras, $rootScope, $scope, ModelItem, ManageSarpras) {
            ...
            $scope.find = function() {
                findSarpras.getOrderBarangUnit('', '', '').then(function(e) {
                    $scope.dataOrder = new kendo.data.DataSource({
            ...
            $scope.findData = function() {
                ...
                if ($scope.from !== undefined && $scope.until !== undefined) {
                    findSarpras.getOrderBarangUnit(moment($scope.from).format('YYYY-MM-DD'), moment($scope.until).format('YYYY-MM-DD'), '').then(function(e) {
                        $scope.dataOrder = new kendo.data.DataSource({
                            data: ModelItem.beforePost(e.data.data)
                        });
