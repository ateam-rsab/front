define(['initialize'], function(initialize) {
    'use strict';
    initialize.controller('MasterBarangProduksiCtrl', ['$q', '$rootScope', '$scope', 'ManageLogistikPhp','$state','CacheHelper',
        function($q, $rootScope, $scope,manageLogistikPhp,$state,cacheHelper) {
            ...
            $scope.item.kdProd = 'PRD/' + moment(new Date()).format('YYMM')+'____'
            var pegawaiUser = {}
            ...
            // init();
            function LoadCache(){
                var chacePeriode = cacheHelper.get('MasterBarangProduksi');
           ...
           function init() {
            // debugger;
            manageLogistikPhp.getDataTableTransaksi("produksi/get-data-combo", true).then(function(dat){
                ...
                if (statusLoad != '') {
                    if (statusLoad == 'Edit') {
                        manageLogistikPhp.getDataTableTransaksi("produksi/get-detail-master-barang-produksi?idprodukhasil="+noreckirim+
                            '&ruid=50', true).then(function(data_ih){
                            ...
                            $scope.listSatuanProd=[{id:data_ih.data.head[0].ssid,satuanstandar:data_ih.data.head[0].satuanview}]
                            $scope.item.produkProd = {id:data_ih.data.head[0].produkhasilfk,namaproduk:data_ih.data.head[0].namaprodukhasil}
                            $scope.item.satuanProd = {id:data_ih.data.head[0].ssid,satuanstandar:data_ih.data.head[0].satuanview}
                            $scope.item.qtyHasil = data_ih.data.head[0].qtyhasil
                            $scope.item.keterangan = data_ih.data.head[0].keterangan
        ...
        // ];
            ...
            $scope.simpan = function(){
                ...
                var objSave = 
                    {
                        produkhasilfk: $scope.item.produkProd.id,
                        qtyhasil: $scope.item.qtyHasil,
                        keterangan:$scope.item.keterangan,
                        details:data2
                    }
                
                manageLogistikPhp.postmasterbarangproduksi(objSave).then(function(e) {
                    $scope.saveShow=true;
