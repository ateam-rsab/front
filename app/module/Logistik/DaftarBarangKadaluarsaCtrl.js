        ...
        function ($sce, $rootScope, $scope, ModelItem, DateHelper, manageLogistikPhp, $http, modelItemAkuntansi, ReportService) {
            ...
            function loadgrid() {
                ...
                if (!$scope.item.ruangan) {
                    toastr.info("Harap pilih ruangan terlebih dahulu");
                    $scope.isRouteLoading = false;
                    return;
                }
                ...
                ReportService.baseLogistik("get-barang-kadaluarsa?tglAwal="+ tglAwal +"&tglAkhir="+ tglAkhir +"&namaProduk=" + ($scope.item.produk ? $scope.item.produk.namaproduk : "") + "&ruanganfk=" + $scope.item.ruangan.id).then((res) => {
                    console.log(res);
                })
