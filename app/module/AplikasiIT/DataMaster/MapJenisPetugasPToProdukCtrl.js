...
define(['initialize'], function(initialize) {
	'use strict';
initialize.controller('MapJenisPetugasPToProdukCtrl', ['$q', '$rootScope', '$scope', 'IPSRSService',
 function($q, $rootScope, $scope,IPSRSService) {
 $scope.item = {};
 $scope.dataVOloaded = true;
 $scope.now = new Date();
 var init = function () {
IPSRSService.getFieldsMasterTable("get-data-master?className=MapJenisPetugasPToProduk", true).then(function(dat){
$scope.listDataMaster = dat.data.data.MapJenisPetugasPToProduk;
...
///colom tabel
$scope.columnMapJenisPetugasPToProduk = [
			{
				"field": "No",
				"title": "No"
			},
...
$scope.mainGridOptions = { 
 pageable: true,
 columns: $scope.columnMapJenisPetugasPToProduk,
 editable: "popup",
 selectable: "row",
 scrollable: false
 };
...
$scope.disableData=function(){
 IPSRSService.getClassMaster("delete-master-table?className=MapJenisPetugasPToProduk&&id="+$scope.item.id+"&&statusEnabled=false").then(function(dat){
 init();
 });
 };
$scope.enableData=function(){
 IPSRSService.getClassMaster("delete-master-table?className=MapJenisPetugasPToProduk&&id="+$scope.item.id+"&&statusEnabled=true").then(function(dat){
 init();
