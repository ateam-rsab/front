define(['initialize'], function(initialize) {
	'use strict';
initialize.controller('ObjekPajakCtrl', ['$q', '$rootScope', '$scope', 'IPSRSService',
 function($q, $rootScope, $scope,IPSRSService) {
 $scope.item = {};
 $scope.dataVOloaded = true;
 $scope.now = new Date();
 var init = function () {
IPSRSService.getFieldsMasterTable("get-data-master?className=ObjekPajak", true).then(function(dat){
$scope.listDataMaster = dat.data.data.ObjekPajak;
...
$scope.mainGridOptions = { 
 pageable: true,
 columns: $scope.columnObjekPajak,
 editable: "popup",
 selectable: "row",
 scrollable: false
 };
...
$scope.item.kdObjekPajak = current.kdObjekPajak;
$scope.item.pajak = current.pajak;
$scope.item.pajakId = current.pajakId;
...
"kdObjekPajak": $scope.item.kdObjekPajak,
"pajak": $scope.item.pajak,
"pajakId": $scope.item.pajakId,
...
IPSRSService.getFieldListData("Pajak&select=id,namaExternal", true).then(function(dat){
$scope.listpajak= dat.data;
});
