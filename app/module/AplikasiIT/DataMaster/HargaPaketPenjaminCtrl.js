define(['initialize'], function(initialize) {
	'use strict';
initialize.controller('HargaPaketPenjaminCtrl', ['$q', '$rootScope', '$scope', 'IPSRSService',
 function($q, $rootScope, $scope,IPSRSService) {
 $scope.item = {};
 $scope.dataVOloaded = true;
 $scope.now = new Date();
 var init = function () {
IPSRSService.getFieldsMasterTable("get-data-master?className=HargaPaketPenjamin", true).then(function(dat){
$scope.listDataMaster = dat.data.data.HargaPaketPenjamin;
...
$scope.mainGridOptions = { 
 pageable: true,
 columns: $scope.columnHargaPaketPenjamin,
 editable: "popup",
 selectable: "row",
 scrollable: false
 };
...
$scope.disableData=function(){
 IPSRSService.getClassMaster("delete-master-table?className=HargaPaketPenjamin&&id="+$scope.item.id+"&&statusEnabled=false").then(function(dat){
 init();
 });
 };
$scope.enableData=function(){
 IPSRSService.getClassMaster("delete-master-table?className=HargaPaketPenjamin&&id="+$scope.item.id+"&&statusEnabled=true").then(function(dat){
 init();
