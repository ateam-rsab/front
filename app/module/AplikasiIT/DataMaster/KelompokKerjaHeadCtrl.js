...
initialize.controller('KelompokKerjaHeadCtrl', ['$q', '$rootScope', '$scope', 'IPSRSService', 'ManagePhp', 
...
function loadData(){
	...
	ManagePhp.getData("generic/get-kelompok-kerja-heads?"+ namaKelompokKerja).then(function(dat){
		// debugger
...
$scope.klik = function(current){
	...
	$scope.item.kdKelompokKerja = Number(current.kdkelompokkerjahead);
	$scope.item.kelompokKerja = current.kelompokkerjahead;
	$scope.item.qKelompokKerja = current.qkelompokkerjahead;
...
function disableData(e){
 		...
 		ManagePhp.getData("kelompok-kerja-head/update-status-enabled-kelompok-kerja-head?id="+dataItem.id+"&statusenabled=false").then(function(dat){
...
function enableData(e){
		...
		e.preventDefault();
			...
			ManagePhp.getData("kelompok-kerja-head/update-status-enabled-kelompok-kerja-head?id="+dataItem.id+"&statusenabled=true").then(function(dat){
...
$scope.simpan = function(){
	...
	var objSave =
		...
		ManagePhp.postData2("kelompok-kerja-head/simpan-kelompok-kerja-head",objSave).then(function (e) {
