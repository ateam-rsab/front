define(['initialize', 'Configuration'], function (initialize, configuration) {
    ...
    initialize.controller('KendaliDokumenRekamMedisRevCtrl', ['$q', '$scope', 'ManageSarprasPhp', 'ModelItem', '$state', '$rootScope', '$timeout', '$window', 'ManagePasien', 'FindPasien', 'DateHelper', 'CetakHelper', 'ModelItem',
        function ($q, $scope, manageSarprasPhp, modelItem, $state, $rootScope, $timeout, $window, managePasien, findPasien, dateHelper, cetakHelper, ModelItem) {
            ...
            $scope.titleTracing = "Daftar Tracing Rekam Medis"
            ...
            $scope.getDataTracingRM = () => {
                // http://192.168.12.3:5555/simrs_harkit/service/transaksi/dokumenrm/get-daftar-tracing?tglAwal=2020-11-18%2000:00:00&tglAkhir=2020-11-18%2023:59:00&jmlRows=50&deptId=18&ruangId=255&kelId=5&dokId=233&sttts=LAMA&noreg=2011009293&norm=01606346&nama=maleeha
                // registrasi-pelayanan/get-daftar-kendali-dokumen-RM?noCm=
                manageSarprasPhp.getDataTableTransaksi("dokumenrm/get-daftar-tracing?tglAwal=" + (dateHelper.formatDate($scope.item.periodeAwal, "YYYY-MM-DD HH:mm:ss")) + "&tglAkhir=" + (dateHelper.formatDate($scope.item.periodeAkhir, "YYYY-MM-DD HH:mm:ss")) + "&jmlRows=50&deptId=&ruangId=&kelId=&dokId=&sttts=&noreg=&norm=" + $scope.item.noCm + "&nama=").then((res) => {
                    console.log("tracing => ", res);
                })
            }
            // $scope.getDataTracingRM();
            ...
            $scope.editGrid = function () {
                if ($scope.current == undefined) {
                    ...
                    manageSarprasPhp.getDataTableTransaksi("dokumenrm/get-data?+norec=" + $scope.current.norec_kdr, false).then(function (data) {
                        ...
                        $scope.item.statusKendaliDokumen = {
                            id: $scope.listStatus[6].id,
                            name: $scope.listStatus[6].name
                        };
                        $scope.item.ruangan2 = {
                            id: datakendali.objectruanganasalfk,
                            namaruangan: datakendali.ruanganasal
                        }
                        $scope.item.ruangan3 = {
                            id: datakendali.objectruangantujuanfk,
                            namaruangan: datakendali.ruangantujuan
                        }
            ...
            $scope.kl = function (current) {
                if (current != undefined) {
                    ...
                    $scope.item.statusKendaliDokumen = {
                        id: current.objectstatuskendalidokumenfk,
                        name: current.name
                    };
                    $scope.item.ruangan2 = {
                        id: current.objectruanganasalfk,
                        namaruangan: current.ruanganasal
                    };
                    $scope.item.ruangan3 = {
                        id: current.objectruangantujuanfk,
                        namaruangan: current.ruangantujuan
                    };
                    $scope.tglJamSkrg = new Date();
                ...
                manageSarprasPhp.getDataTableTransaksi("dokumenrm/get-data-kendali?+nocm=" + $scope.noCm +
                    ...
                    if (datakendali.length == 0) {
                        $scope.dataItem = {
                            tglUpdate: $scope.now,
                            ruangan: {
                                id: data.data.rak[0].id,
                                namaruangan: data.data.rak[0].namaruangan
                            },
                            statusKendaliDokumen: {
                                id: $scope.listStatus[0].id,
                                name: $scope.listStatus[0].name
                            }
                        }
                        ...
                        $scope.dataItem = {
                            ...
                            ruangan1: {
                                id: datakendali[0].kdruangan,
                                namaruangan: datakendali[0].ruangdaftar
                            },
                            statusKendaliDokumen: {
                                id: $scope.listStatus[0].id,
                                name: $scope.listStatus[0].name
