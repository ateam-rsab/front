define(['initialize'], function(initialize) {
    'use strict';
    initialize.controller('ReportPasienCtrl', ['$interval', 'SaveToWindow', '$rootScope', '$scope', 'ModelItem', '$state', 'FindPasien', 'DateHelper', 'socket', 'ManagePasien', '$window',
        function($interval, saveToWindow, $rootScope, $scope, ModelItem, $state, findPasien, dateHelper, socket, managePasien, $window) {
            ...
            $scope.kl = function(item){
				$scope.item = item;
				var item = ModelItem.beforePost(item, true);
				$scope.item.pasien.noCm = item.pasien.noCm;
				$scope.dateTglInput = new Date(item.pasien.tglDaftar);
				$scope.item.tglInput = dateHelper.getTanggalFormattedNew($scope.dateTglInput);
			};	

			$scope.report = function(){
	            $window.open('http://172.16.16.45:8080/jasamedika-web/reporting/lapPengkajianAwalRawatJalan?noCm='+$scope.item.pasien.noCm+'&tglInput='+$scope.item.tglInput+'&X-AUTH-TOKEN=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJvcGVyYXRvciJ9.eNEJVKwi8thRx0ruZxlk377mgpgkkLGtraRKwBjmb3Y4yT_nxfWUCeT-DrJ93_U0ZNJZrc-TM2rO4cxe5LjL5A', '_blank');
	        };
