define(['initialize'], function(initialize) {
    'use strict';
    initialize.controller('DaftarAntrianPasienOperatorRevCtrl', ['SaveToWindow', '$rootScope', '$scope', 'ModelItem', '$state', 'FindPasien', 'DateHelper', 'socket', 'ManagePasien', '$mdDialog', '$window', 'CetakHelper', 'ManageSarprasPhp','CacheHelper',
        function(saveToWindow, $rootScope, $scope, ModelItem, $state, findPasien, dateHelper, socket, managePasien, $mdDialog, window, cetakHelper, manageSarprasPhp,cacheHelper) {
            ...
            $scope.batalPeriksa = function() {
                ...
                if ( $scope.item.pasienDaftar.noRegistrasi != undefined) {
                    norReg = "noReg=" + $scope.item.pasienDaftar.noRegistrasi;
                }
            ...
            $scope.lanjutBatal = function(data, batal) {
                ...
                manageSarprasPhp.saveDataTransaksi('operator/save-Batal-registrasi',BatalPeriksa).then(function(e){
                    $scope.items = {};
                    $scope.winDialog.close();
                })
                //  var listRawRequired = [
            ...
            $scope.CetakSumList = function() {
                if($scope.item != undefined){
                     ...
                     //##save summarylist
                    manageSarprasPhp.getDataTableTransaksi("operator/identifikasi-sum-list?norec_pd="
                        +  $scope.item.pasienDaftar.noRec
                         ).then(function(data) {
                            var datas=data.data;
                    })
                    //##end 
            ...
            $scope.cetakBuktiLayanan = function() {
                if($scope.item != undefined){   
                    ...
                    //##save buktilayanan
                    manageSarprasPhp.getDataTableTransaksi("operator/identifikasi-buktiLayanan?norec_pd="
                        +  $scope.item.pasienDaftar.noRec
                         ).then(function(data) {
                            var datas=data.data;
                    })
            ...
            $scope.tracerBon = function() {
                if($scope.item != undefined){
                    ...
                    //##save tracer
                    manageSarprasPhp.getDataTableTransaksi("operator/identifikasi-tracer?norec_pd="
                        +  $scope.item.pasienDaftar.noRec
                         ).then(function(data) {
                            var datas=data.data;
                    })
            ...
            $scope.pilihQty = function(data) {
                ...
                if(isValid.status){
                    ...
                    if(qty !== undefined){
                         ...
                         //##save label
                        manageSarprasPhp.getDataTableTransaksi("operator/identifikasi-label?norec_pd="
                            +  $scope.item.pasienDaftar.noRec +'&islabel=' + qtyhasil
                             ).then(function(data) {
                              var datas=data.data;
                        }) 
                        //##end
            ...
            $scope.cetakBro = function(){
                ...
                manageSarprasPhp.getDataTableTransaksi("operator/identifikasi-rmk?norec_pd="
                    +  $scope.item.pasienDaftar.noRec
                     ).then(function(data) {
                        var datas=data.data;
                })
                ...
                if ($scope.item){
                    ...
                    // });
                     $state.go('InputTindakanPelayananRev',{
                        norecPD : $scope.item.pasienDaftar.noRec,
                        norecAPD:  $scope.item.noRec   
                    }); 
                } else {
            ...
            $scope.cetakSEP = function() {
                if($scope.item != undefined && $scope.item.kelompokPasien !== "Umum/Pribadi"){
                    ...
                    //cetakan langsung service VB6 by grh    
                    var client = new HttpClient();
                    client.get('http://127.0.0.1:1237/printvb/Pendaftaran?cetak-sep-new=1&norec=' + $scope.item.pasienDaftar.noRegistrasi + '&view=false', function(response) {
                        // do something with response
                    });
                }
            }
            ...
            $scope.editRegistrasi = function(){
                    ...
                    //     noRec: $scope.item.pasienDaftar.noRec
                    // });
                 manageSarprasPhp.getDataTableTransaksi("registrasipasien/get-apd?noregistrasi="
                        +$scope.item.pasienDaftar.noRegistrasi
                        + "&objectruanganlastfk="+$scope.item.ruangan.id
                    ).then(function (data) {
                        ...
                        var cacheSet =$scope.item.pasienDaftar.noRec
                                         + "~" + $scope.item.pasienDaftar.noRegistrasi
        ...
        $scope.EditPemakaianAsuransi = function(){
             manageSarprasPhp.getDataTableTransaksi("registrasipasien/get-apd?noregistrasi="
                        ...
                        var dataAntrian = data.data.data[0];
                        manageSarprasPhp.getDataTableTransaksi("registrasipasien/get-pemakaian-asuransi?noregistrasi="
                            +$scope.item.pasienDaftar.noRegistrasi
                             ).then(function (data) {
