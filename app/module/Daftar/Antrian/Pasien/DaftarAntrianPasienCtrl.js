define(['initialize'], function(initialize) {
    'use strict';
    initialize.controller('DaftarAntrianPasienCtrl', ['$interval', 'SaveToWindow', '$rootScope', '$scope', 'ModelItem', '$state', 'FindPasien', 'DateHelper', 'socket', 'ManagePasien', 'FindPegawai', 'ModelItemAkuntansi',
        function($interval, saveToWindow, $rootScope, $scope, ModelItem, $state, findPasien, dateHelper, socket, managePasien, findPegawai, ModelItemAkuntansi){
            ...
            if (ModelItem.getPegawai().ruangan !== undefined){
                $scope.statusPegawai = ModelItem.getPegawai().ruangan.id;
            } else {
                $scope.statusPegawai = 35;
            }
            ...
            //     $scope.ruanganRJ = e.data.data;
            // });
              ModelItemAkuntansi.getDataTableTransaksi("registrasipasien/get-data-combo", true)
                        .then(function (dat) {
                            debugger
                            for (var i =  dat.ruanganrajal.length - 1; i >= 0; i--) {
                                 dat.ruanganrajal[i].namaRuangan = dat.ruanganrajal[i].namaruangan 
                            }
                            $scope.ruanganRJ = dat.ruanganrajal;
             })
            ...
            $scope.findData = function() {
                ...
                findPasien.findQueue($scope.from, $scope.until, $scope.ruangan, $scope.namaPasien, pegawai).then(function(e) {
            ...
            $scope.pilihDokter = function(item, data) {
                ...
                //     });
                // })
                ModelItemAkuntansi.getDataTableTransaksi("pasien/get-dokters-combos", false).then(function(data) {
                    $scope.listDokter = data.dokter;
            ...
            $scope.masukPasien = function(data) {
                ...
                if (objValid.status) {
                    ...
                    if (!data.dokter) {
                        ...
                        debugger;
                        console.log(JSON.stringify(objValid))
                        if (objValid.tglDipanggilDokter)
                            objValid.tglDipanggilDokter = dateHelper.formatDate(objValid.tglDipanggilDokter, 'YYYY-MM-DD HH:mm:ss');
                            // objValid.tglDipanggilDokter = new Date().toISOString();
                        if (objValid.tglDipanggilSuster)
                            objValid.tglDipanggilSuster = dateHelper.formatDate(objValid.tglDipanggilSuster, 'YYYY-MM-DD HH:mm:ss');
                        //    objValid.tglDipanggilSuster = new Date().toISOString();
                        console.log(JSON.stringify(objValid))

                        managePasien.updateStatusAntrian(data.noRec, objValid.statusAntrian, objValid.tglDipanggilSuster, objValid.tglDipanggilDokter).then(function() {
                            if (statusCode === 'suster') {
                                window.messageContainer.log('Data Berhasil Disimpan');
                    ...
                    case "suster":
                        switch (statusAntrian) {
                            ...
                            case "MENUNGGU":
                                obj.status = true;
                                obj.statusAntrian = 1;
                                obj.tglDipanggilSuster = dateHelper.getDateTimeFormatted($scope.now);
                                break;
                        };
                        break;
                    case "dokter":
                        switch (statusAntrian) {
                            ...
                            case "DIPANGGIL_SUSTER":
                                obj.status = true;
                                obj.statusAntrian = 2;
                                obj.tglDipanggilDokter = dateHelper.getDateTimeFormatted($scope.now);
                                break;
                            case "MENUNGGU":
                                obj.msg = "Pasien harus di panggil suster terlebih dahulu";
                                obj.status = false;
