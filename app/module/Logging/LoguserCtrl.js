define(['initialize'], function(initialize) {
  ...
  initialize.controller('LoguserCtrl', ['$rootScope', '$scope', 'ModelItem','$state','DateHelper','ManageLogInfo','$mdDialog','$element',
    function($rootScope, $scope, ModelItem,$state,DateHelper,ManageLogInfo,$mdDialog,$element) {
      $scope.item = {};
      ModelItem.get("Logging/Loguser").then(function(data) {
        $scope.item = data;
   ...
   $scope.source = data;
      ...
      var titles = JSON.parse(itemPegawai);
       ...
       $scope.klik = function(dataSelected){
        debugger;
        var dataSelected2;
        $scope.item.no3 = dataSelected.no3;
        dataSelected.isCRUD;
        if(dataSelected.isCRUD=="I"){
           dataSelected2 = 'Insert'
        }else if (dataSelected.isCRUD=="U"){
           dataSelected2 = 'Update'
        }else{
           dataSelected2 = 'Delete'
        }
        $scope.item.isCRUD = dataSelected2;
      ...
      $scope.openWindow = function(dataSelected){
        debugger;

         if ($scope.item.isCRUD==null){
        alert("Daftar Modul Operasi Belum dipilih, Klik terlebih dahulu !!");
      }else{
