define(['initialize'], function(initialize) {
	'use strict';
	initialize.controller('RealisasiTargetCtrl', ['$rootScope', '$scope', 'ModelItem',
		function($rootScope, $scope, ModelItem, DateHelper, $document, r) {		

			$scope.dataVOloaded = true;
			$scope.item = {};
			$scope.RealisasiTarget = new kendo.data.DataSource({
				data: [
				{
			...
			$scope.now = new Date();
			ModelItem.get("InformasidanPerencanaanRumahSakit/RealisasiTarget").then(function(data) {
				$scope.item = data;
				scope.dataVOloaded = true;
