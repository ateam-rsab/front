define(['initialize'], function(initialize) {
    'use strict';
    initialize.controller('TidurIstirahatCtrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'FindPasien', 'CacheHelper', 'ManagePasien', 'DateHelper',
        function($q, $rootScope, $scope, ModelItem, $state, findPasien, cacheHelper, ManagePasien, dateHelper) {
            ...
            $q.all([ModelItem.getDataDummyGeneric("StatusLamaTidur", false),
                findPasien.getByNoCM($scope.noCM)
            ]).then(function(data) {
                debugger;
                if (data[0].statResponse)
                    $scope.listStatusLamaTidur = data[0];
            ...
            $scope.Save = function() {
                ...
                ManagePasien.saveTidurIstirahat(pasienDaftar,ModelItem.beforePost($scope.pasien), dateHelper.toTimeStamp($state.params.tanggal), ModelItem.beforePost($scope.item)).then(function(e) {
