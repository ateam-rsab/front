	...
	initialize.controller('DaftarKirimMenuCtrl', ['$mdDialog', '$timeout', '$state', '$q', '$rootScope', '$scope', 'CacheHelper', 'DateHelper', 'ManageServicePhp',
		function ($mdDialog, $timeout, $state, $q, $rootScope, $scope, cacheHelper, dateHelper, manageServicePhp) {
			...
			$scope.ubahKirim = function () {
				...
				$scope.dataPasienSelected.details.forEach(el => {
					$scope.jsonDataPasien.push({
						"norec_op": el.norec_op,
						"norec_pd": el.norec_pd,
						"objectjeniswaktufk": el.objectjeniswaktufk
					});
				});

				console.log($scope.jsonDataPasien);


				// "norec_op": "e7b06830-f9ed-11e9-9f57-f32b24b7",
				// "norec_pd": "0c0b4550-ea9c-11e9-aa33-59c3c0df",
				// "objectjeniswaktufk": ""
			...
			$scope.saveKirimMenu = function () {
				...
				var objSave = {
					...
					"details": data2,
					"datapasien": $scope.jsonDataPasien
				}
				var post = {
					"strukkirim": objSave
				}
