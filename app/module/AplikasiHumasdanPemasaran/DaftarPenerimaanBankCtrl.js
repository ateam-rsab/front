...
define(['initialize'], function(initialize) {
	'use strict';
	initialize.controller('DaftarPenerimaanBankCtrl', ['$rootScope', '$scope', 'ModelItem', 'ManageSdm',  'DateHelper','InformasiRuangan','InformasiKelompokTransaksi','InformasiPegawaiTujuan','InformasiPegawaiPenerima','$mdDialog', '$state',
		function($rootScope, $scope, ModelItem, ManageSdm, DateHelper,InformasiRuangan,InformasiKelompokTransaksi,InformasiPegawaiTujuan,InformasiPegawaiPenerima,$mdDialog,$state) {
			...
			$scope.now = new Date();
			 ...
			 $scope.listNamabankAccount = [

             {
             "id":1, "namaExternal":"BRI_6306"
             },
             {
              "id":2, "namaExternal":"BRI_2"
             },
              {
              "id":3, "namaExternal":"EDC_BII"
             },
             {
              "id":4, "namaExternal":"EDC_BNI"
             }
            ...
            ManageSdm.getItem("terimakeluarkaskecil/get-filter-data-by-from-to-bankin?from="+tglAwal+"&to="+tglAkhir, true).then(function(dat){
            ...
            //khusus refres daftar penerimaan bank
            ...
            //daftar tampil awal penerimaan bank
            var tglAwal = moment($scope.item.tanggalawal).format('YYYY-MM-DD');
		 	var tglAkhir = moment($scope.item.tanggalakhir).format('YYYY-MM-DD');
		    ManageSdm.getItem("terimakeluarkaskecil/get-filter-data-by-from-to-bankin?from="+tglAwal+"&to="+tglAkhir, true).then(function(dat){
			$scope.dataMaster = dat.data.data.data;
            ...
            $scope.PenerimaanBank = function(){
             	debugger
             $state.go("DaftarPenerimaanBank",
					{
     ...
     $scope.openWindow2 = function(){   
      ...
      $scope.tambah = function(){    
       ...
       $scope.isShowPopUp3 = true;
        ...
        InformasiPegawaiTujuan.getOrderList("planningdiklathumasmarketpeserta/get-list-all-pegawai", true).then(function(dat){
         ...
         InformasiPegawaiPenerima.getOrderList("planningdiklathumasmarketpeserta/get-list-all-pegawai", true).then(function(dat){
            ...
            $scope.item.kelompokTransaksifilter = "PENERIMAAN BANK";
       ...
       $scope.cancelData3 = function(){
		...
		$scope.Save = function () {
			debugger;
			if($scope.item.banks.bankAccId==undefined){
				alert("Bank Wajib di pilih !!")
			}else{
	      var data = {
						    "nonHistori" : "KK17070002",
						    "tglHistori" : $scope.item.tanggalhistory,
						    "tglSetorTarikDeposit" : $scope.item.tanggal,
						    "bankAccount" : {"id" : $scope.item.banks.bankAccId},
