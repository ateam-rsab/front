define(['initialize'], function(initialize) {
    'use strict';
    initialize.controller('RegistrasiPenunjangCtrl', ['$q', '$rootScope', '$scope', 'ManageLogistikPhp','$state','CacheHelper',
        ...
        function init() {
          ...
          $scope.$watch('item.kelompokpasien', function(e) {
                if (e === undefined) return;
                    manageLogistikPhp.getDataTableTransaksi("registrasipasien/get-penjaminbykelompokpasien?kdKelompokPasien="+e.id)   
                      .then(function (z) {
                        $scope.listPenjamin=z.data.rekanan;
                        if (e.kelompokpasien=='Umum/Pribadi')
                        {
                             $scope.item.namaPenjamin={id:$scope.listPenjamin[0].id,namarekanan:$scope.listPenjamin[0].namarekanan};
                             // $scope.item.namaPenjamin='';
                             $scope.nonUmum=false;                  
                        }
        ...
        $scope.Simpan= function(){
            ...
            if ($scope.item.kelompokpasien == undefined) {
                 toastr.error("Pilih kelompokpasien terlebih dahulu!!")
                return
            }
            if ($scope.item.asalRujukan == undefined) {
                 toastr.error("Pilih asalRujukan terlebih dahulu!!")
                return
            }
            if (data2.length == 0) {
            ...
            // var jenisPenjaminId=null
            // if ($scope.item.jenisPenjamin != undefined) {
            //    jenisPenjaminId =$scope.item.jenisPenjamin.id
            // }
            if ($scope.item.namaPenjamin == undefined) {
                toastr.error('Pilih Nama Penjamin terlebih dahulu!!')
                return
            }
            ...
            var objSaveRegistrasi = {
                ...
                objectruanganfk:$scope.item.ruangan.id,
                statuspasien: 'Baru',
                objectrekananfk:$scope.item.namaPenjamin.id ,
            }
