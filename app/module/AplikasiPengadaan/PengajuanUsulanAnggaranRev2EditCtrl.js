define(['initialize'], function(initialize) {
	'use strict';
	initialize.controller('PengajuanUsulanAnggaranRev2EditCtrl', ['$rootScope', '$scope', '$state', 'ModelItem', 'DateHelper','PengajuanUsulanAnggaranService','FindPasien',
		function($rootScope, $scope, $state, ModelItem, DateHelper, PengajuanUsulanAnggaranService,findPasien) {
			debugger;
			...
			debugger;
			findPasien.getEditUsulanAnggaran($state.params.noRec).then(function(dat){
				debugger;
				var thehead = dat.data.result[0].head
				$scope.dataPenyusunanTRPNBPs = dat.data.result[0].detailSpek
				for (var i = 0; i < thehead.length; i++) {
					thehead[i]
				$scope.item.deskripsi = dat.data.result[0].paketUsulan[0].namaPaketUsulan
				$scope.item.tahun = thehead[i].tahun
				$scope.item.kegiatanDetail = {"id":thehead[i].kegiatanDetailId}
				$scope.item.output = {"id":thehead[i].outputId}
				$scope.item.jenisPengadaanLengkap = {"id":thehead[i].jenisPengadaanId}
				$scope.item.komponen = {"id":thehead[i].komponenId}
				$scope.item.jenisBelanjaLengkap = {"id":thehead[i].jenisBelanjaId}
				$scope.item.akun = {"id":thehead[i].mataAnggaranId}
				$scope.item.asalProduk = {"id":thehead[i].asalProdukId}
				}
				$scope.dataPenyusunanTRPNBP = new kendo.data.DataSource({
                    data: $scope.dataPenyusunanTRPNBPs,
                    serverPaging: false,
                    pageSize: 12,
                    schema:  {
                        model: {
    ...
    //         	{
				...
				{ 	
					command: { text: "Tampilkan", click: testos } }
				// {
		 ...
		 //    };
            ...
            function testos() {
            	debugger;
            	$scope.item.spesifikasi = $scope.dataPasienSelected.noRec;
            }
			/*
			...
			$scope.pushSpekAnggaranBaru = function(dataSpekSet){
                ...
                if(isValid.status){
					...
					var newData = {
						...
						"jenisBelanja": $scope.item.jenisBelanjaLengkap,
						"pengendali": dataSpekSet.pengendali,
						"jenisPengadaan": $scope.item.jenisPengadaanLengkap
					};
			...
			$scope.SimpanFinal = function (){
				if ($scope.SpekAnggaranSet._total !== 0) {
	                ...
	                if(isValid.status){
						...
						PengajuanUsulanAnggaranService.savePengajuan(ModelItem.beforePost($scope.item),"anggaran/edit-pengajuan-usulan-anggaran/?noRec="+$state.params.noRec).then(function(e){	
							debugger;
