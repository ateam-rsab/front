define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('JadwalRencanaPemeriksaanCtrl', ['$rootScope', '$scope', '$state', 'ModelItem', 'DateHelper', 'ManageSarpras', 'JadwalRencanaPemeriksaanService', '$document', 'R',
		function ($rootScope, $scope, $state, ModelItem, DateHelper, ManageSarpras, JadwalRencanaPemeriksaanService, $document, r) {
			...
			$scope.findSatuanKerja = function () {
				ManageSarpras.getOrderList("jadwal-rencana-pemeriksaan/find-all-by-date-satuan-kerja/" + bln + "/" + $scope.now.getFullYear() + "/" + $scope.item.satuanKerja.id).then(function (data) {
					$scope.jadwalSupir = [];
					console.log(bln);
