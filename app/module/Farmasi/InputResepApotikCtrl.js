define(["initialize"], function (initialize) {
  "use strict";
  initialize.controller("InputResepApotikCtrl", [
    "$q",
    "$rootScope",
    "$scope",
    "ManageLogistikPhp",
    ...
    function ($q, $rootScope, $scope, manageLogistikPhp, $state, cacheHelper) {
      ...
      LoadCache();
      function LoadCache() {
        // manageLogistikPhp.getDataTableTransaksi("akutansi/get-terakhir-posting", true).then(function(dat){
        ...
        // })
        manageLogistikPhp
          .getDataTableTransaksi("akutansi/get-tgl-posting", true)
          .then(function (dat) {
        ...
        if (chacePeriode != undefined) {
          ...
          $scope.item.kelompokPasien = chacePeriode[12];
          $scope.item.umur = chacePeriode[4];

          $scope.listKelas = [
            { id: chacePeriode[5], namakelas: chacePeriode[6] },
          ];
          $scope.item.kelas = {
            id: chacePeriode[5],
      ...
      function init() {
        ...
        // debugger;
        manageLogistikPhp
          .getDataTableTransaksi("logistik/get-datacombo", true)
          .then(function (dat) {
            $scope.isRouteLoading = false;
              ...
              if (noOrder == "EditResep") {
                manageLogistikPhp
                  .getDataTableTransaksi(
                    "logistik/get_detail-resep?norecResep=" + norecResep,
                    true
                  )
                  .then(function (data_ih) {
                    ...
                    $scope.item.totalSubTotal = parseFloat(subTotal)
                      .toFixed(2)
                      .replace(/(\d)(?=(\d{3})+\.)/g, "$1,");
                  });
              } else {
                manageLogistikPhp
                  .getDataTableTransaksi(
                    "logistik/get-detail-order?noorder=" + noOrder,
                    true
                  )
                  .then(function (dat) {
                    $scope.isRouteLoading = false;
      ...
      $scope.getSatuan = function () {
        ...
        manageLogistikPhp
          .getDataTableTransaksi(
            "logistik/get-info-stok?produkfk=" + $scope.item.produk.id,
            true
          )
          .then(function (e) {
      ...
      function GETKONVERSI() {
        ...
        manageLogistikPhp
          .getDataTableTransaksi(
            "logistik/get-produkdetail?" +
              "produkfk=" +
              $scope.item.produk.id +
              "&ruanganfk=" +
          ...
          .then(function (dat) {
            ...
            if ($scope.dataSelected != undefined) {
              ...
              $scope.item.jumlahxmakan =
                parseFloat($scope.dataSelected.jumlah) /
                parseFloat($scope.item.dosis);
              $scope.item.nilaiKonversi = $scope.dataSelected.nilaikonversi;
              $scope.item.satuan = {
      ...
      $scope.$watch("item.jumlahxmakan", function (newValue, oldValue) {
        if (newValue != oldValue) {
          if ($scope.item.stok > 0) {
            $scope.item.jumlah =
              parseFloat($scope.item.jumlahxmakan) *
              parseFloat($scope.item.dosis);
          }
        }
      });
      ...
      $scope.$watch("item.hargadiskon", function (newValue, oldValue) {
        if (newValue != oldValue) {
          ...
          $scope.item.total =
            parseFloat($scope.item.jumlah) * (hrg1 - hrgsdk) +
            parseFloat(tarifJasa);
        }
      });
      ...
      $scope.$watch("item.jumlah", function (newValue, oldValue) {
        if (newValue != oldValue) {
          ...
          for (var i = 0; i < dataProdukDetail.length; i++) {
            ...
            if (
              parseFloat(
                $scope.item.jumlah * parseFloat($scope.item.nilaiKonversi)
              ) <= parseFloat(dataProdukDetail[i].qtyproduk)
            ) {
              hrg1 = Math.round(
                parseFloat(dataProdukDetail[i].hargajual) *
                  parseFloat($scope.item.nilaiKonversi)
              );
              hrgsdk =
                parseFloat(dataProdukDetail[i].hargadiscount) *
                parseFloat($scope.item.nilaiKonversi);
              $scope.item.hargaSatuan = hrg1;
              $scope.item.hargaNetto = Math.round(
                parseFloat(dataProdukDetail[i].harganetto) *
                  parseFloat($scope.item.nilaiKonversi)
              );
        ...
        if ($scope.item.no != undefined) {
          for (var i = data2.length - 1; i >= 0; i--) {
            if (data2[i].no == $scope.item.no) {
              ...
              data.tglregistrasi = moment($scope.item.tglregistrasi).format(
                "YYYY-MM-DD hh:mm:ss"
              );
              data.generik = null;
              ...
              data.jmldosis =
                String($scope.item.jumlah / dosis) + "/" + String(dosis);
              data.jasa = tarifJasa;
          ...
          data = {
            ...
            tglregistrasi: moment($scope.item.tglregistrasi).format(
              "YYYY-MM-DD hh:mm:ss"
            ),
            generik: null,
      ...
      $scope.klikGrid = function (dataSelected) {
        ...
        manageLogistikPhp
          .getDataTableTransaksi(
            "farmasi/get-jenis-obat?jrid=" + dataSelected.jenisobatfk,
            true
          )
          .then(function (JR) {
      ...
      $scope.simpan = function () {
        ...
        for (var i = data2.length - 1; i >= 0; i--) {
          if (data2[i].hargasatuan == 0 && data2[i].jmlstok == 0) {
            alert(
              "Terdapat obat dengan harga kosong, kemungkinan stock kosong!!"
            );
            return;
          }
        }
        ...
        $scope.tombolSimpanVis = false;
        manageLogistikPhp.postpelayananapotik(objSave).then(
          function (e) {
            $scope.tombolSimpanVis = true;
            ...
            manageLogistikPhp
              .getDataTableTransaksi(
                "logging/save-log-input-resep?norec_apd=" +
                  norec_apd +
                  "&penulisresepfk=" +
            ...
            client.get(
              "http://127.0.0.1:1237/printvb/farmasiApotik?cetak-strukresep=" +
                $scope.item.consisid +
                "&nores=" +
                e.data.noresep.norec +
                "&view=" +
                stt +
                "&user=" +
                pegawaiUser.namalengkap,
              function (response) {
                //aadc=response;
              }
            );
