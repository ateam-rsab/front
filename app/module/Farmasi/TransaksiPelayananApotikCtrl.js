define(['initialize'], function(initialize) {
    'use strict';
    initialize.controller('TransaksiPelayananApotikCtrl', ['$q', '$rootScope', '$scope', 'ManageLogistikPhp','$state','CacheHelper',
        function($q, $rootScope, $scope,manageLogistikPhp,$state,cacheHelper) {
           ...
           function init() {
            ...
            if (detail == "detail") {
                manageLogistikPhp.getDataTableTransaksi("logistik/get-transaksi-pelayanan?nocm="+$scope.item.noMr+"&noregistrasifk="+norec_apd, true).then(function(dat){
                    $scope.isRouteLoading=false;
                ...
                if(chacePeriode != undefined){
                   var noresep=chacePeriode[0]
                    manageLogistikPhp.getDataTableTransaksi("logistik/get-transaksi-pelayanan?nocm="+$scope.item.noMr+"&norec_resep="+noresep, true).then(function(dat){
                        $scope.isRouteLoading=false;
                    for (var i = 0; i < dat.data.length; i++) {
                        dat.data[i].no = i+1
                        dat.data[i].total =parseFloat(dat.data[i].jumlah) * (parseFloat(dat.data[i].hargasatuan)-parseFloat(dat.data[i].hargadiscount))
                ...
                manageLogistikPhp.getDataTableTransaksi("logistik/get-transaksi-pelayanan?nocm="+$scope.item.noMr, true).then(function(dat){
                    $scope.isRouteLoading=false;
                    for (var i = 0; i < dat.data.length; i++) {
                        dat.data[i].no = i+1
                        dat.data[i].total =parseFloat(dat.data[i].jumlah) * (parseFloat(dat.data[i].hargasatuan)-parseFloat(dat.data[i].hargadiscount))
        ...
        $scope.data2 = function(dataItem) {
            ...
            $scope.paketObat =function(){
                ...
                $scope.dataGrid = [];
                manageLogistikPhp.getDataTableTransaksi("logistik-stok/get-daftar-paket-obat-pasien?"+
            ...
            $scope.ubahPopUp = function(){
                ...
                var chacePeriode ={ 0 : $scope.item.noMr ,
                                    1 : $scope.item.namaPasien,
                                    2 : $scope.item.jenisKelamin,
                                    3 : $scope.item.noregistrasi, 
                                    4 : $scope.item.umur,
                ...
                manageLogistikPhp.getDataTableTransaksi("tatarekening/get-sudah-verif?noregistrasi="+
                    ...
                    if (dat.data.status == true) {
                        ...
                        manageLogistikPhp.posthapuspelayananapotik(objDelete).then(function(e) {
                         ...
                         //##save Logging user
                       manageLogistikPhp.getDataTableTransaksi("logging/save-log-hapus-resep?norec_resep="
