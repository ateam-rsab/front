define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('MasterTunjanganBahayaRadiasiCtrl', ['$rootScope', '$scope',
		...
		function ($rootScope, $scope, ModelItem, $state,
            ...
            ManageSdm.getOrderList("service/list-generic/?view=LevelTingkat&select=id,levelTingkat", true).then(function(dat) {
            ...
            function initial(){
                ...
                ManageSdm.getOrderList("pegawai-ltja/get-all-pegawai-ltja", true).then(function(dat) {
                    $scope.gridMasterTunjanganBahayaRadiasi = dat.data;
                });
                 $scope.item = {};
                  $scope.vals = false;
            }
			$scope.columnGridMasterTunjanganBahayaRadiasi ={ 
				pageable: true,
             ...
             $scope.klik = function(current) {
                ...
                for (x = 0; x < $scope.listLevelTingkat.length; x++) {
                    if ($scope.listLevelTingkat[x].id === current.levelTingkatId) {
            ...
            $scope.Save = function () { 
                ...
                if($scope.item.komponenHarga === undefined || $scope.item.operatorFactorRate === undefined || 
                    $scope.item.levelTingkat  === undefined || $scope.item.komponenHarga === undefined 
                ...
                var data = {
                    ...
                    "levelTingkatId": $scope.item.levelTingkat.id,
                    "namaLevelTingkat": $scope.item.levelTingkat.levelTingkat, 
                ...
                ManageSdm.saveJenisSantunan(ModelItem.beforePost(data), "pegawai-ltja/save-pegawai-ltja").then(function(e) {
                    initial();                 
                });
            };
