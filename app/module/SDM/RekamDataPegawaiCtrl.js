        ...
        function ($q, $rootScope, $scope, ModelItem, $state, ManageSdm, ManageSdmNew, dateHelper, FindPegawai, FindSdm, $timeout, manageSarprasPhp, modelItemAkuntansi, $mdDialog) {
            ...
            $scope.init = function () {
                ...
                $q.all([
                    ...
                    $scope.SumberData = [
                        { sumberData: '-', id: 0 },
                        { sumberData: 'Fungsional', id: 2 },
                        { sumberData: 'Internal', id: 1 },
                        { sumberData: 'Struktural', id: 3 }
                    ];
                    if (res[0].data) {
            ...
            $scope.getEvaluasiJabatanPegawaiBaru = () => {
                ...
                ManageSdmNew.getListData("map-pegawai-jabatan-unitkerja/get-evaluasi-jabatan-pegawai-baru?listJabatanId=" + listJabatanSelected).then(res => {
                    var isEmptyModel = _.isEmpty(res.data.data.data);
                    if (!isEmptyModel) {
                        $scope.item.nilaiJabatan = res.data.data.data.nilaiJabatan;
                        $scope.item.grade = res.data.data.data.grade;
                        $scope.item.detailKelompokJabatan = {
                            id: res.data.data.data.idDetailKelompokJabatan,
                            detailKelompokJabatan: res.data.data.data.detailKelompokJabatan
                        }
                    }
            ...
            $scope.simpanJabatanInternal = function () {
                ...
                for (var key in newModel[0]) {
                    if (newModel[0].hasOwnProperty(key)) {
                        // if (key.indexOf('analisaJabatan') >= 0) {
                        //     delete newModel[0][key];
                        // }

                        if (key.indexOf('sumberData') == 0 && $scope.ji.sumberData.id != 0) {
                            newModel[0]['sumberData'] = $scope.ji.sumberData.id;
                        } else if (key.indexOf('sumberData') == 0) {
                            delete newModel[0][key]
                        }
            ...
            $scope.getDataSubUnitKerjaById = function (id, data, jenisId) {
                ...
                $("#idComboDataRJabatan").data("kendoComboBox").value("");
            ...
            function editDataJabatanInternal(e) {
                ...
                clearPop();
                // if (dataItem.kdJabatan) {
                //     $scope.ji.analisaJabatan = dataItem.kdJabatan
                // }
                if ($scope.item.detailKelompokJabatan) {
                    if ($scope.item.detailKelompokJabatan.detailKelompokJabatan.toUpperCase() === 'DIREKTUR UTAMA (DIRUT)' ||
                        $scope.item.detailKelompokJabatan.detailKelompokJabatan.toUpperCase() === 'DIREKTUR UTAMA' ||
                ...
                $scope.ji.sumberData = {
                    ...
                    sumberData: dataItem.sumberData.sumberData
                };
            ...
            function clearPop() {
                ...
                $scope.isDireksi = false;
                $scope.isStaff = true;
                // $scope.ji.analisaJabatan = null;
                $scope.ji.sumberData = null;
                $scope.ji.jabatan = "";
                $scope.listJabatanByJenisJabatanInternal = []
                $scope.ji.jenisJabatan = '';
                $scope.ji.pejabatPenilai = '';
            ...
            $scope.simpanRiwayatJabatan = function () {
                ...
                var dataSave = {
                    ...
                    "statusEnabled": true,
                    "sumberData": $scope.item.sumberData.id,
                    "jenisJabatan": {
            ...
            $scope.createNewRiwayatJabatan = function () {
                ...
                $scope.isEditedRJ = false;

                $scope.item.noSK = '';
                $scope.item.tglSK = '';
                $scope.item.sumberData = '';
            ...
            function changeRowJabatan(e) {
                ...
                // acTtdSk.destroy();
                // acJabatan.destroy();
                dataItem.unitKerjaPegawai = {
                    id: dataItem.unitKerjaId,
                    name: dataItem.unitKerja
                }
                $scope.getDataSubUnitKerjaById(dataItem.unitKerjaPegawai.id, dataItem.unitKerjaPegawai, dataItem.idJenisJabatan);
                $scope.getDataJabatan(dataItem.idJenisJabatan);
                $scope.noRecRiwayatJabatan = dataItem.noRec;
                if (dataItem.sumberData == null) {
                    $scope.item.sumberData = null
                } else {
                    $scope.item.sumberData = {
                        id: dataItem.sumberDataId,
                        sumberData: dataItem.sumberData
                    }
                }
                $scope.item.riwayatJabatan = {
