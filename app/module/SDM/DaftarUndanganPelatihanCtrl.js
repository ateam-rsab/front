	...
	initialize.controller('DaftarUndanganPelatihanCtrl', ['$rootScope', '$scope', 'ModelItem', '$state', 'ModelItemAkuntansi', 'CacheHelper', 'ManageServicePhp', 'DateHelper',
		function ($rootScope, $scope, ModelItem, $state, modelItemAkuntansi, cacheHelper, manageServicePhp, dateHelper) {
			...
			$scope.CetakUndangan = function () {
				...
				client.get('http://127.0.0.1:1237/printvb/sdm?cetak-undangan-pelatihan=1&nores=' + Pelatihan + '&strIdPegawai=' + $scope.dataSelected.pegawaifk + '&view=' + stt + '&noPlanning=' + $scope.dataSelected.noplanning, function (response) {
			...
			$scope.CetakSuratTugas = function () {
				...
				var client = new HttpClient();
				client.get('http://127.0.0.1:1237/printvb/sdm?cetak-surat-tugas=1&nores=' + Pelatihan + '&strIdPegawai=' + $scope.dataSelected.pegawaifk + '&view=' + stt + '&noPlanning=' + $scope.dataSelected.noplanning, function (response) {
			...
			$scope.CetakPerNara = function () {
				...
				var client = new HttpClient();
				client.get('http://127.0.0.1:1237/printvb/sdm?cetak-permohonan-narasumber=1&nores=' + Pelatihan + '&strIdPegawai=' + $scope.dataSelected.pegawaifk + '&view=' + stt + '&noPlanning=' + $scope.dataSelected.noplanning, function (response) {
			...
			$scope.CetakPerMC = function () {
				...
				var client = new HttpClient();
				client.get('http://127.0.0.1:1237/printvb/sdm?cetak-permohonan-mc=1&nores=' + Pelatihan + '&strIdPegawai=' + $scope.dataSelected.pegawaifk + '&view=' + stt + '&noPlanning=' + $scope.dataSelected.noplanning, function (response) {
