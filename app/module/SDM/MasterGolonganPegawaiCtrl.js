define(['initialize'], function(initialize) {
	'use strict';
	initialize.controller('MasterGolonganPegawaiCtrl', ['$rootScope', '$scope', 'ModelItem', 'ManagePegawai', 'FindPegawai', '$timeout',
		function($rootScope, $scope, ModelItem, ManagePegawai, FindPegawai, $timeout) {
			$scope.title = "Master Golongan Pegawai";
			$scope.opsiGrid = {
				...
				columns: [
					...
					{field: "name", title: "Golongan Pegawai"},
			...
			function init(){
				...
				FindPegawai.getListData("sdm/get-all-golongan").then(function(res){
							...
							model: {
								...
								fields: {
									...
									name: {editable: true, validation: { 
										...
										if (input.is("[name='golongan']") && input.val() === "") {                        
											return false;
										}
										return true;
										}
			...
			$scope.Save = function(data){
				...
				ManagePegawai.saveMasterData("sdm/save-golongan", item).then(function(res){
					// debugger;
				}, (error) => {
					throw error;
				})
			...
			var timeoutPromise;
			$scope.$watch('item.namaGolongan', function(newVal, oldVal){
			...
			function applyFilter(filterField, filterValue){
				var gridData = $("#gridGolongan").data("kendoGrid");
				var currFilterObject = gridData.dataSource.filter();
