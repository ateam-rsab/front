define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('MasterAkreditasiPelatihanCtrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'ManagePhp', '$timeout',
		function ($q, $rootScope, $scope, ModelItem, $state, managePhp, $timeout) {
			...
			$scope.daftarMasterOpt = {
				...
				columns: [
					...
					{ field: "akreditasipelatihan", title: "Akreditasi Pelatihan" },
			...
			function init() {
				...
				$q.all([
					managePhp.getMaster("pelatihan/get-daftar-akreditasi-pelatihan", true),
				]).then(function (res) {
					if (res[0].statResponse) {
						$scope.dataSource = new kendo.data.DataSource({
							data: res[0].data.data,
							sort: {
								field: "akreditasipelatihan",
								dir: "asc"
							},
							pageSize: 20,
							schema: {
									...
									fields: {
										...
										akreditasipelatihan: {
											editable: true, validation: {
												validasiakreditasipelatihan: function (input) {
													if (input.is("[name='akreditasipelatihan']") && input.val() === "") {
														return false;
													}
													return true;
												}
							...
							change: function (e) {
								...
								if (e.action === "remove") {
									var item = e.items[0];
									if (item.akreditasipelatihan !== "" && item.id !== "") {
			...
			$scope.Save = function (data) {
				...
				managePhp.postMaster(item,'pelatihan/save-akreditasi-pelatihan').then(function (e) {
					// delete $scope.item;
					// $scope.item = {};
					// init();
				});
			...
			var timeoutPromise;
			$scope.$watch('item.akreditasiPelatihan', function (newVal, oldVal) {
				$timeout.cancel(timeoutPromise);
				timeoutPromise = $timeout(function () {
					if (newVal != oldVal) {
						applyFilter("akreditasipelatihan", newVal)
					}
				}, 500)
			})
