define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('LapBIOSPenerimaanKeuanganCtrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'ManageSdm', 'ManageSdmNew', 'ReportService', 'DateHelper', 'FindPegawai', 'FindSdm', '$timeout', '$mdDialog',
        function ($q, $rootScope, $scope, ModelItem, $state, ManageSdm, ManageSdmNew, ReportService, dateHelper, FindPegawai, FindSdm, $timeout, $mdDialog) {
            ...
            var initIndikator = function () {
                $scope.gridIndikator = {
                    toolbar: [{
                        name: "create",
                        text: "Entri Data Penerimaan Keuangan",
                    ...
                    excel: {
                        allPages: true,
                        fileName: "RSAB HK Export Data Penerimaan Keuangan - " + dateHelper.formatDate(new Date(), 'DD-MMM-YYYY HH:mm:ss') + ".xlsx"
                    },
                    excelExport: function (e) {
                        ...
                        var myHeaders = [{
                            value: "Data Penerimaan Keuangan Periode " + dateHelper.formatDate($scope.periode, 'MMM YYYY'),
                            fontSize: 14,
                            textAlign: "center",
                            background: "#ffffff",
                        }];
            ...
            $scope.loadDataGridIndikator = function () {
                ...
                ReportService.getListData("reporting/transaksi-bios-indikator-lain?bulan=" + periode + "&tglAwal=" + tglAwal + "&tglAkhir=" + tglAkhir + "&idIndikator=" + idIndikator + "&statusKirim=" + statusKirim + "&jenisIndikator=2").then(function (data) {
            ...
            $scope.init = function () {
                $q.all([
                    ManageSdmNew.getListData("pelayanan/indikator-penerimaan-keuangan")
                ]).then(function (res) {
                    if (res[0].data) {
            ...
            $scope.sentToBIOSG2 = function (data) {
                ...
                ManageSdmNew.getListData("integrasi/send-to-bios?bulan=" + dateHelper.formatDate(data.tglT, "YYYY-MM") + "&noRec=" + data.noRec + "&jenisIndikator=2").then(function () {
                    $scope.isRouteLoading = false;
                    $scope.loadData();
                }, (error) => {
            ...
            $scope.sentAllToBIOSG2 = function (e) {
                ...
                $mdDialog.show(confirm).then(function () {
                    ...
                    ManageSdmNew.getListData("integrasi/send-to-bios?bulan=" + dateHelper.formatDate($scope.data[0].tglT, "YYYY-MM") + "&jenisIndikator=2").then(function (response) {
                        $scope.isRouteLoading = false;
