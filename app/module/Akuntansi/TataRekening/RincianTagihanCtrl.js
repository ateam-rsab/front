define(['initialize'], function(initialize) {
	'use strict';
	initialize.controller('RincianTagihanCtrl', ['$sce', '$state', '$q', '$rootScope', '$scope', '$window', 'ModelItemAkuntansi', 'ManageTataRekening','ManageKasir', 'ManagePasien', '$mdDialog',
	    			...
	    			manageTataRekening.saveakomodasitea(objSave).then(function(data){
						...
						.then(function(data) {
							...
							if (data[0].statResponse){
								...
								data[0].bayar=dibayar
								data[0].verifTotal=verifTotal
								data[0].sisa=parseFloat(data[0].billing) - dibayar - parseFloat(data[0].deposit) 
								$scope.item = data[0];
    ...
    //                 return;
           ...
           $scope.showKomponenHarga = function(){
            ...
            function loadKomponen(){
            	manageTataRekening.getDataTableTransaksi("tatarekening/get-komponenharga-pelayanan?norec_pp="+$scope.dataSelected.norec).
					  then(function(data){
