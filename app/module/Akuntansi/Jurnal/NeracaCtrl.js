define(['initialize'], function(initialize) {
    'use strict';
    initialize.controller('NeracaCtrl', ['$q', '$rootScope', '$scope', 'ManageAkuntansi','$state','CacheHelper','DateHelper',
        function($q, $rootScope, $scope,manageAkuntansi,$state,cacheHelper,dateHelper) {
            ...
            $scope.listLevel =[{id:1,level:1},{id:2,level:2},{id:3,level:3}]
            $scope.item.level = {id:2,level:2};
            LoadCache();
            loadCombo();
            function LoadCache(){
            ...
            function init() {
                ...
                // }
                var level =""
                if ($scope.item.level != undefined){
                    var level ="&level=" +$scope.item.level.id
                }
                var bulanName = dateHelper.formatDate($scope.item.bulan,"MMMM")
                ...
                var tgltglmin = moment($scope.item.bulan).subtract(1, 'months')
                var bulanMinName = dateHelper.formatDate(tgltglmin,"MMMM")
                var bulanMin = dateHelper.formatDate(tgltglmin,"MM")
                var tahunMin = dateHelper.formatDate(tgltglmin,"YYYY")
                var tgltglminus = tahunMin + bulanMin

                var tglAwal1 = tahun+"-"+bulan+"-01"
                var tglAkhir1 = tahun+"-"+bulan+"-"+getLastDay( tahun,bulan)
                manageAkuntansi.getDataTableTransaksi("akuntansi/get-data-neraca?"+
                    "tglAwal=" + tglAwal1 + 
                    "&tglAkhir=" + tglAkhir1 +
                    "&tgltgl=" + tgltgl +
                    "&reportdisplay=neraca" + level +
                    "&tglmin=" + tgltglminus
                    ...
                    $scope.optionsDataGrid = {
                        ...
                        columns: [
                            ...
                            },
                            {
                                ...
                                "title": bulanMinName,
                                "width" : "90px",
                                "template": "<span class='style-right'>{{formatRupiah('#: totalbefore #', '')}}</span>"
                            },
                            {
                                "field": "total",
                                "title": bulanName,
