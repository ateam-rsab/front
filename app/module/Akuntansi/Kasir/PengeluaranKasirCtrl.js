define(['initialize'], function(initialize) {
	'use strict';
	initialize.controller('PengeluaranKasirCtrl', ['$mdDialog', '$state', '$q', '$rootScope', '$scope', 'ModelItemAkuntansi', 'ManageKasir',
    ...
    //             if($scope.showCC)
			...
			$scope.setDefaultNominal = function(){
				return ($scope.item.jumlahBayarFix - $scope.item.totalFixBayar)*-1;
			}
			...
			.then(function(data) {
				...
				if (data[0].statResponse){
					...
					$scope.item.diskon = 0;
					$scope.item.totalTagihan = $scope.item.jumlahBayar*-1;
					$scope.item.jumlahBayarFix = (($scope.item.totalTagihan - $scope.item.diskon) + $scope.item.biayaAdministrasi)*-1;
					$scope.item.totalFixBayar = 0;
