	...
	initialize.controller('PenerimaanBridgingCtrl', ['$sce', '$q', '$rootScope', '$scope', '$http', 'ModelItemAkuntansi', 'ManageAkuntansi','DateHelper','CacheHelper',
		function($sce, $q, $rootScope, $scope, $http, modelItemAkuntansi, manageAkuntansi,dateHelper,cacheHelper) {
			...
			$scope.init = () => {
					...
					manageAkuntansi.getDataTableTransaksi(`akuntansi/get-penerimaan-bridge?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}`)
					.then((res)=>{
			...
			$scope.cariFilter = () => {
				...
				manageAkuntansi.getDataTableTransaksi(`akuntansi/get-penerimaan-bridge?tglAwal=${tglParAwal}&tglAkhir=${tglParAkhir}`).then((res) => {
