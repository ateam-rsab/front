define(['initialize'], function (initialize) {
    ...
    initialize.controller('LaporanPasienPulangCtrl', ['CacheHelper', '$q', '$rootScope', '$scope', 'ModelItemAkuntansi', 'DateHelper', '$http', '$state', 'ReportPelayanan', 'ManageSdm', 'ManageLogistikPhp', 'FindPasien', 'ManageTataRekening',
        function (cacheHelper, $q, $rootScope, $scope, modelItemAkuntansi, DateHelper, $http, $state, ReportPelayanan, ManageSdm, manageLogistikPhp, findPasien, manageTataRekening) {
            ...
            $scope.CetakRekapitukasiPenjamin = () => {
                let tglAwal = DateHelper.formatDate($scope.item.tglawal, "YYYY-MM-DD");
                let tglAkhir = DateHelper.formatDate($scope.item.tglakhir, "YYYY-MM-DD");
                let URL = `http://192.168.12.4:7777/service-reporting/lap-rekap-penjamin?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}&idDept=${$scope.item.departement.id}&kelompokPasien=${$scope.item.namaPenjamin.id}`;
                if ($scope.item.institusiAsalPasien && !$scope.item.institusiAsalPasien.length) {
                    URL = `http://192.168.12.4:7777/service-reporting/lap-rekap-penjamin-by-institusi?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}&idDept=${$scope.item.departement.id}&kelompokPasien=${$scope.item.namaPenjamin.id}&institusiPasien=${$scope.item.namaPenjamin.id === 2 ?  $scope.item.institusiAsalPasien[0].id : $scope.item.institusiAsalPasien.id}`
                }
                // http://192.168.12.4/:7777/service-reporting/lap-rekap-penjamin-by-ranap?tglAwal=2020-06-01&tglAkhir=2020-07-31&idDept=18&kelompokPasien=5
                window.open(URL, '_blank');
            }

            $scope.cetakRekapPenjaminRanap = () => {
                let tglAwal = DateHelper.formatDate($scope.item.tglawal, "YYYY-MM-DD");
                let tglAkhir = DateHelper.formatDate($scope.item.tglakhir, "YYYY-MM-DD");
                let _URL = `http://192.168.12.4:7777/service-reporting/lap-rekap-penjamin-by-ranap?tglAwal=${tglAwal}&tglAkhir=${tglAkhir}&idDept=${$scope.item.departement.id}&kelompokPasien=${$scope.item.namaPenjamin.id}`;
