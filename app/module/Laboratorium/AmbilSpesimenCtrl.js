define(['initialize'], function(initialize) {
    'use strict';
    initialize.controller('AmbilSpesimenCtrl', ['ManagePasien', 'ManageLaboratorium', '$rootScope', '$scope', 'ModelItem', '$state', 'RegistrasiPasienBaru', 'FindPasien', 'FindPasienLaboratorium', 'DateHelper',
        ...
        function(managePasien, manageLaboratorium, $rootScope, $scope, ModelItem, $state, RegistrasiPasienBaru, findPasien, findPasienLaboratorium, dateHelper) {
            ...
            $scope.cetak = function() {
                $state.go('LaboratoriumCetakBarcodeCtrl', $state.params);
            }
            $scope.listSpesimens = [];
            findPasienLaboratorium.getOrder($scope.noOrder).then(function(data) {
                var items = data.data.data.orders[0].detail;
