define(['kendo.menu'], function(kendoMenu) {
    ...
    function HeaderCtrl($window, $scope, MenuService, GlobalToState, $rootScope, $state, LoginHelper, $location, socket) {
        ...
        MenuService.getDataMenuPhp("pegawai/svc-modul?get=loginuser&id="+$scope.datauserlogin.id).then(function(data){
            $scope.userLogin = data.data.loginuser[0];
            $scope.dataConfirmUser =  $scope.userLogin.namauser; 
            $scope.dataConfirmKelompokUser =  $scope.userLogin.kelompokuser;     
        })
        ...
        $scope.sendEditKataSandi = function(){
            ...
            var objSave = {
                        "id":$scope.datauserlogin.id,
                        "kelompokUser":{  
                          id: $scope.userLogin.kuid
                        },
                        "statusLogin":0,
                        "kataSandi":$scope.dataConfirmPassword,
                        "namaUser":$scope.userLogin.namauser,
                        "pegawai":{  
                          id:$scope.dataPegawai.id
                        }
            }
               MenuService.saveLoginUser(objSave)
