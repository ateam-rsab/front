define(['initialize'], function(initialize) {
    initialize.controller('WelcomeCtrl', ['$state', 'socket', '$window', '$location', '$scope', 'R', '$rootScope', 'MenuService', 'LoginHelper', 'ManageServicePhp',  'CacheHelper',
        function($state, socket, $window, $location, $scope, r, $rootScope, MenuService, LoginHelper, manageServicePhp, CacheHelper) {
        ...
        // *change egie*
            manageServicePhp.getDataTableTransaksi("pegawai/svc-modul?get=loginuser&id="+$scope.datauserlogin.id).then(function(data){
                    $scope.kelompokUser = data.data.loginuser[0];
            })
            ...
            $scope.showSubMenu = function(id, urlIconModul, nama, isLogin){              
                ...
                manageServicePhp.getDataTableTransaksi("pegawai/get-all-objek-modul?idModulAplikasi="+$scope.modulaplikasiid +"&idKelompokUser="
                    +  $scope.kelompokUser.kuid)
           // MenuService.getDataMenu("modul-aplikasi/get-all-objek-modul/"+$scope.datauserlogin.id+"/"+$scope.modulaplikasiid+"/show")
                ...
                var arrCache = { 0 : $scope.modulaplikasiid ,
                                 1 : $scope.kelompokUser.kuid
                }
                CacheHelper.set('WelcomeCtrl', arrCache);   
            }
                 // *end change egie*
